...
class edm extends Controller
{
    ...
    public function mail_action()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('您所访问的资源不存在'));
        }
    }
    ...
    public function unsubscription_action()
    {
        ...
        if (md5($email . G_SECUKEY) == $arg[1])
        {
            if ($human_verify == ip2long(fetch_ip()))
            {
                ...
                H::redirect_msg(Application::lang()->_t('%s 退订邮件成功', $email));
            }
            else
            {
                $unsubscription_link = get_js_url('/account/edm/unsubscription/' . $arg[0] . ',' . $arg[1] . ',' . ip2long(fetch_ip()));
                
                H::redirect_msg(Application::lang()->_t('是否确认退订邮件订阅? &nbsp; ( <a href="%s">继续</a> )', $unsubscription_link));
            }
        }
        else
        {
            H::redirect_msg(Application::lang()->_t('退订链接无效'));
        }
    }
}