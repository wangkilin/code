...
class main extends SinhoBaseController
{
    ...
    public function verify_list_action ()
    {
        ...
        // 按照书稿，类别， 遍次排序。
        if ($bookIds) {
            $allList = $this->model('sinhoWorkload')->fetch_all(sinhoWorkloadModel::WORKLOAD_TABLE, 'book_id IN (' . join(',', $bookIds) . ') AND (status != ' . sinhoWorkloadModel::STATUS_DELETE . ' )', 'book_id,weight,category,working_times');
        }
    ...
    protected function check_by_user (& $userList, & $bookList)
    {
        ...
        $totalRows = $this->model('sinhoWorkload')->found_rows();
        $bookIds = array_unique(array_column($allList, 'book_id'));
        // 根据书稿id列表获取书稿信息
        $bookList = array();
        if ($bookIds) {
        ...
        if (isset($_POST['export_user_workload']) && $_POST['export_user_workload']=='1') {
            ...
            $fileName = '导出编辑工作量-' . date('Y-m-d') . '.xls';
            $itemList = array();
            $recordingLines = $verifyingLines = $payedLines = array();
            $_bookLine = 2;
            foreach ($allList as $_item2) {
                $_item2['category'          ] = $bookList[$_item2['book_id']]['category'          ];
                ...
                if ($_item2['add_time']) {
                    ...
                    $_item2['date'] = substr($bookList[$_item2['book_id']]['delivery_date'], 5);
                }
                $_item2['date'] .= '~';
                if ($_item2['fill_time']) {
                ...
                $_item2['status'] == 3 AND $recordingLines[] = $_bookLine++;
            }
