        ...
        <div class="mod-head">
            ...
            <div class="dropdown pull-right">
                <a class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown">
                    <?php if ($this->account_info['id'] === 0) { _e('主账号'); } else { _e('子账号 %s', $this->account_info['id']); } ?>
                ...
                <ul class="dropdown-menu weixin-dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                    <?php foreach ($this->accounts_list AS $account_info) {
                        if ($account_info['id'] == $this->account_info['id']) { continue; } ?>
        ...
        <div class="tab-content mod-body">
            ...
            <div class="table-responsive">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                    <input type="hidden" name="account_id" value="<?php echo $this->account_info['id']; ?>" />
                    ...
                    <tr>
                        ...
                        <td><?php if (!$this->echo_rule_list[$rule_info['keyword']]) { ?><input class="flag-action" type="radio" value="<?php echo $rule_info['keyword']; ?>" name="is_subscribe"<?php if ($this->account_info['weixin_subscribe_message_key'] == $rule_info['keyword']) { ?> checked="checked"<?php } ?> /><?php } else { ?>-<?php } ?></td>
                        <td><?php if (!$this->echo_rule_list[$rule_info['keyword']]) { ?><input class="flag-action" type="radio" value="<?php echo $rule_info['keyword']; ?>" name="is_no_result"<?php if ($this->account_info['weixin_no_result_message_key'] == $rule_info['keyword']) { ?> checked="checked"<?php } ?> /><?php } else { ?>-<?php } ?></td>
                        <td class="nowrap">
                ...
                <div class="mod-table-foot mod-one-btn">
                    <a class="btn btn-primary" href="admin/weixin/reply_edit/account_id-<?php echo $this->account_info['id']; ?>" id="batch_approval"><?php _e('添加规则'); ?></a>
                </div>
            </div>
        </div>
    </div>
