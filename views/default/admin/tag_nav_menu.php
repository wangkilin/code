    ...
    <div class="tab-content mod-content">
        <div class="row">
            ...
            <div class="col-md-7 nopadding">
                <form action="admin/ajax/save_tag_nav_menu/" method="post" id="nav_menu_list" onsubmit="return false">
                    ...
                    <ul data-listidx="0">
                    <?php
                    if ($this->nav_menu_list) {
                      foreach($this->nav_menu_list as $key => $val) {
                        if ($val['title']) { ?>
                        <li data-sort="<?php echo $val['id']; ?>" data-cursor="pointer">
                            <div class="mod-set-head <?php echo $val['parent_id'] ? 'item-intent' : 'item-no-intent' ?>">
                                <span class="pull-right">
                                  ...
                                  <a href="javascript:;" onclick="AWS.dialog('confirm', {'message': '<?php _e('确认删除?'); ?>'}, function(){AWS.ajax_request(G_BASE_URL + '/admin/ajax/remove_tag_nav_menu/', 'id=<?php echo $val['id']; ?>');}); return false;"><?php _e('删除'); ?></a>
                                </span>
                                <h4><?php echo $val['title']; ?> (<?php if ($val['type'] == 'category') { ?><?php _e('分类'); ?><?php } else if ($val['type'] == 'topic') { ?><?php _e('话题'); ?><?php } else { ?><?php _e('自定义链接'); ?><?php } ?>)</h4>
                            </div>
            ...
            <div class="mod">
                <div class="icb-message-box topic">
                    ...
                    <div class="mod-set-body">
                        <form action="admin/ajax/add_tag_nav_menu/" method="post">
