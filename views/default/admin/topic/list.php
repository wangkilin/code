        ...
        <div class="mod-body tab-content">
            <div class="tab-pane active" id="list">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a onclick="ICB.ajax.requestJson(G_BASE_URL + '/admin/ajax/lock_topic/', 'topic_id=<?php echo $topic_info['topic_id']; ?>&status=0');" title="<?php _e('解除锁定'); ?>" data-toggle="tooltip" class="icon icon-lock md-tip"></a>
                                    <?php } else { ?>
                                    <a href="javascript:;" onclick="ICB.ajax.requestJson(G_BASE_URL + '/admin/ajax/lock_topic/', 'topic_id=<?php echo $topic_info['topic_id']; ?>&status=1');" data-toggle="tooltip" title="<?php _e('锁定话题'); ?>" class="icon icon-unlock md-tip"></a>
                                    <?php } ?>
                                </td>

								<td><?php echo $topic_info['sort']; ?></td>

                                <td><?php if ($topic_info['is_hot']) { ?><?php _e('是'); ?><?php } else { ?><?php _e('否'); ?><?php } ?></td>

                                <td class="icb-parent-topic">
                                    <?php if ($topic_info['parent_ids']) {
                                    foreach ($topic_info['parent_ids'] as $_parentId) {?>
                                    <a href="topic/<?php echo $this->parent_topic_list[$_parentId]['url_token']; ?>" target="_blank"><?php echo $this->parent_topic_list[$_parentId]['topic_title']; ?></a>
                                    <?php }
                                    }?>
                                </td>
                ...
                <div class="mod-table-foot">
                    ...
                    <a class="btn btn-danger" onclick="$('#action').val('remove'); ICB.ajax.postForm($('#batchs_form'));"><?php _e('删除话题'); ?></a>
                    <a class="btn btn-primary" onclick="$('#action').val('lock'); ICB.ajax.postForm($('#batchs_form'));"><?php _e('锁定话题'); ?></a>
            ...
            <div class="tab-pane" id="today-topics">
                <form method="post" action="admin/ajax/save_today_topics/" onsubmit="return false;" id="today_topics_form" class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-5 col-xs-8">
                            <button type="button" onclick="ICB.ajax.postForm($('#today_topics_form'));" class="btn btn-primary"><?php _e('保存设置'); ?></button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    $(function(){
        ...
        $('#batchs_post').click(function()
        {
            if($('.icheckbox_square-blue').hasClass("checked")){
                ...
                $('#parent_id').val($("#icb-modal-window .form-control:eq(1)").val());
