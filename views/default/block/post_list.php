	...
	<div class="icb-question-content">
		...
		<p>
			...
			<?php if ($val['answer_info']['anonymous']) { ?>
				<a href="javascript:;" class="icb-user-name"><?php _e('匿名用户'); ?></a><?php } else { ?><a href="user/<?php echo $val['answer_info']['user_info']['url_token']; ?>" class="icb-user-name" data-id="<?php echo $val['answer_info']['user_info']['uid']; ?>"><?php echo $val['answer_info']['user_info']['user_name']; ?></a><?php } ?>
			...
			<?php if ($val['anonymous'] == 0) { ?>
				<a href="user/<?php echo $val['user_info']['url_token']; ?>" class="icb-user-name"><?php echo $val['user_info']['user_name']; ?></a><?php } else { ?><a href="javascript:;" class="icb-user-name" data-id="<?php echo $val['uid']; ?>"><?php _e('匿名用户'); ?></a><?php } ?>
			...
			<?php } ?>
			<?php } else { ?>
				<a href="user/<?php echo $val['user_info']['url_token']; ?>" class="icb-user-name"><?php echo $val['user_info']['user_name']; ?></a> <span class="text-color-999"><?php _e('发表了文章'); ?> • <?php _e('%s 个评论', $val['comments']); ?> • <?php _e('%s 次浏览', $val['views']); ?> • <?php echo date_friendly($val['add_time']); ?></span>
			<?php } ?>
...
<?php if ($this->posts_list) { ?>
<?php foreach ($this->posts_list AS $key => $val) { ?>
<div class="icb-item clearfix <?php if ($val['question_id']) { ?><?php if ($val['answer_count'] == 0) { ?>active<?php } ?><?php } else {?>article<?php } ?>" data-topic-id="<?php foreach ($val['topics'] AS $k => $v) { ?><?php echo $v['topic_id']; ?>,<?php } ?>">
