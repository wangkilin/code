	...
	<div class="row">
	  <div class="col-sm-12">
		<ul class="list">
			...
			<?php foreach ($this->tag_nav_menu as $key => $val) { ?>
			<?php if ($val['title']) { ?>
		...
		<?php if ($val['title']) { ?>
		 <div class="row">
		  <div class="nav-row-title">
		   <span class="title col-sm-12 icon-right">
              <?php echo $val['title'];?>
            </span>
          </div>
		 </div>
		 <div class="row my-category-<?php echo $val['url_token'] ? $val['url_token'] : $val['id']; ?>-and-sub-item">

		  <div class="icb-content-wrap nav-block-row clearfix">
           <div class="col-sm-3  my-category-<?php echo $val['url_token'] ? $val['url_token'] : $val['id']; ?>">
		   ...
		   <?php foreach ($val['child'] AS $_key => $_val) { ?>
		     <div class="col-sm-3 category-sub-item">
	            ...
	            <span class="title"><a href="<?php echo $_val['link']; ?>"<?php if ($_val['type'] == 'custom') { ?> target="_blank"<?php } ?>><?php echo $_val['title']; ?></a></span>
