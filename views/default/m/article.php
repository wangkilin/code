...
<!-- 内容 -->
	<div class="container padding-0">
		<!-- 话题bar -->
		<div class="icb-article-title-box clearfix" data-id="<?php echo $this->article_info['id']; ?>" data-type="article">
			<div class="tag-queue-box">
				...
				<?php } ?>
				<?php if ($this->user_id) { ?>
					<a class="icon icon-inverse icb-add-topic-box"><i class="icon icon-edit"></i> <?php if (!count($this->article_topics) > 0) { ?>添加话题<?php } ?></a>
				<?php } ?>
			</div>
		</div>
		<!-- end 话题bar -->
		...
		<!-- 文章详情 -->
		<div class="icb-question-detail icb-article-detail">
			<div class="mod-head">
			...
			<div class="mod-footer active">
				...
				<span class="icb-article-vote operate <?php if (!$this->user_id OR $this->user_id == $this->article_info['uid']) { ?> disabled<?php } ?>">
				...
				<div class="icb-article-voter">
					...
					<a href="user/<?php echo $val['url_token']; ?>" class="voter" data-toggle="tooltip" data-placement="right" data-original-title="<?php echo $val['user_name']; ?>"><img class="img" width="20" alt="<?php echo $val['user_name']; ?>" src="<?php echo get_avatar_url($val['uid'], 'mid'); ?>" /></a>
					<?php } ?>
		...
		<div class="icb-replay-list">
			...
			<div class="mod-body">
				<ul>
					...
					<li>
						<div class="mod-head">
							...
							<a href="m/user/<?php echo $val['user_info']['url_token']; ?>"><img class="img" alt="" src="<?php echo get_avatar_url($val['uid'], 'mid'); ?>" width="20" /></a>
							<a href="m/user/<?php echo $val['user_info']['url_token']; ?>"><?php echo $val['user_info']['user_name']; ?></a>
							<?php if ($val['at_user_info']) { ?> <?php _e('回复'); ?> <a href="user/<?php echo $val['at_user_info']['url_token']; ?>"><?php echo $val['at_user_info']['user_name']; ?></a><?php } ?>
						</div>
						<div class="mod-body">
							<div class="markitup-box clearfix">
						...
						<div class="mod-footer">
							<div class="meta">
								...
								<a class="icb-article-comment" data-id="<?php echo $val['user_info']['uid']; ?>">回复</a>
								<?php } ?>
							</div>
						</div>
					</li>
				...
				<?php if ($_GET['item_id']) { ?>
				<a href="article/<?php echo $this->article_info['id']; ?>" class="icb-load-more"><?php _e('查看全部评论'); ?></a>
				<?php } ?>
			</div>
			<?php } ?>
		</div>
		...
		<a class="icb-load-more" id="load_index_actions" href="account/login/"><?php _e('登录进行回复'); ?></a>
		<?php } else { ?>
		<!-- 回复box -->
		<div class="icb-replay-box article">
			<form action="article/ajax/save_comment/" onsubmit="return false;" method="post" id="answer_form">
				...
				<div class="mod-head">
					<a href="m/user/"><img width="20" class="img" src="<?php echo get_avatar_url($this->user_id, 'mid'); ?>" alt="" /></a> <?php echo $this->user_info['user_name']; ?>
				</div>
				<div class="mod-body">
