...
<?php } ?>
	...
	<div class="container padding-0">
		<!-- 话题bar -->
		<div class="icb-article-title-box clearfix" data-id="<?php echo $this->question_info['question_id']; ?>" data-type="question">
			<div class="tag-queue-box">
				...
				<?php if ($this->user_id AND !$this->question_info['lock']) { ?>
					<a class="icon icon-inverse icb-add-topic-box"><i class="icon icon-edit"></i> <?php if (!count($this->question_topics) > 0) { ?><?php _e('添加话题'); ?><?php } ?></a>
				<?php } ?>
			</div>
		</div>
		<!-- end 话题bar -->
		...
		<div class="icb-question-detail">
			...
			<div class="mod-footer">
				<div class="meta">
					<a class="icb-invite-replay"><i class="icon icon-inviteask"></i> <?php _e('邀请回复'); ?></a>
				...
				<div class="icb-invite-box collapse">
					<div class="mod-head users-list">
						...
						<?php foreach($this->invite_users as $key => $val) { ?>
							<a href="user/<?php echo $val['url_token']; ?>" data-id="<?php echo $val['uid']; ?>"><img class="img" width="20" src="<?php echo get_avatar_url($val['uid'], 'mid'); ?>" /></a>
						<?php } ?>
						<?php } ?>
					</div>
					<div class="mod-body">
						...
						<div class="icb-dropdown-list collapse">
							<ul></ul>
						</div>
					</div>
				</div>
				...
				<ul>
					...
					<li uninterested_count="<?php echo $val['uninterested_count']; ?>" force_fold="<?php if ($val['user_rated_uninterested']) { ?>1<?php } else { ?><?php echo $val['force_fold']; ?><?php } ?>" id="answer_list_<?php echo $val['answer_id']; ?>">
						<div class="mod-head">
							...
							<!-- end 最佳回答 -->
							<?php } ?>
							<a href="<?php if ($val['anonymous']) { ?>javascript:;<?php } else { ?>m/user/<?php echo $val['uid']; ?><?php } ?>"><img class="img" width="20" src="<?php if ($val['anonymous']) { ?><?php echo G_STATIC_URL; ?>/common/avatar-mid-img.png<?php } else { ?><?php echo get_avatar_url($val['uid'], 'mid'); ?><?php } ?>" alt="" /></a>
							<a href="<?php if ($val['anonymous']) { ?>javascript:;<?php } else { ?>m/user/<?php echo $val['uid']; ?><?php } ?>"><?php if ($val['anonymous']) { ?><?php _e('匿名用户'); ?><?php } else { ?><?php echo $val['user_info']['user_name']; ?><?php } ?></a><?php if (!$val['anonymous']) { ?>
						...
						<div class="mod-footer">
							<div class="meta">
								...
								<?php if ($this->user_id) { ?>
								<span class="operate">
									<a class="icb-add-comment" data-id="<?php echo $val['answer_id']; ?>" data-type="answer" <?php if ($this->question_info['lock'] OR !$this->user_info['permission']['publish_comment']) { ?> data-close="true"<?php } ?>><i class="icon icon-comment"></i> <?php echo $val['comment_count']; ?></a>
								</span>
								<span class="operate">
		...
		<!-- 折叠回复 -->
		<div class="icb-replay-list uninterest">
		...
		<!-- 回复box -->
		<div class="icb-replay-box question">
			<a name="answer_form"></a>
			<?php if ($_GET['single']) { ?>
			<a href="question/<?php echo $this->question_info['question_id']; ?>" class="icb-load-more"><?php _e('查看全部回答'); ?></a>
			<?php } else if ($this->next_page) { ?>
			<a href="m/question/<?php echo $this->question_info['question_id']; ?>?page=<?php echo $this->next_page; ?>" class="icb-load-more"><?php _e('下一页'); ?></a>
			<?php } ?>

			<?php if (!$this->user_id) { ?>
			<a class="icb-load-more no-border" id="load_index_actions" href="account/login/"><?php _e('登录进行回复'); ?></a>
			...
			<form action="question/ajax/save_answer/" onsubmit="return false;" method="post" id="answer_form" class="question_answer_form">
				...
				<div class="mod-head">
					<a href="m/user/"><img class="img" width="20" src="<?php echo get_avatar_url($this->user_id, 'mid'); ?>" alt="" /></a>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		// 折叠回复
		$.each($('li.icb-item'), function (i, e) {
			if ($(this).attr('uninterested_count') >= UNINTERESTED_COUNT || $(this).attr('force_fold') == 1)
			{
				$('#uninterested_answers_list').append('<li class="icb-item">' + $(e).html() + '</li>');
		...
		//折叠回复
		$.each($('.icb-replay-list ul li'), function (i, e)
		{
			if ($(this).attr('uninterested_count') >= UNINTERESTED_COUNT || $(this).attr('force_fold') == 1)
			{
