...
class accountModel extends Model
{
    ...
    {
        $result =  $db_password == compile_password($password, $salt);
    ...
    public function get_user_info_by_username($user_name, $attrb = false, $cache_result = true)
    {
        ...
        {
            return $this->getUserById($uid, $attrb, $cache_result);
        }
    }
     ...
     * @return array
     */
    public function getUsersByIds($uids, $attrib = false, $withPassword = false, $debug=false)
    {
        if (! is_array($uids) OR count($uids) == 0) {
            return false;
        }
...
//         if (count($uids) == 1) {
//             if ($userInfo = $this->getUserById(end($uids), $attrib)) {
//                 return array(
//                     end($uids) => $userInfo
//                 );
//             }
...
//        }
        ...
        } else if (! $attrib && isset($users_info[implode('_', $uids)])) {
            return $users_info[implode('_', $uids)];
        }

        $where = count($uids)==1 ? ('uid='.end($uids)) : "uid IN(" . implode(',', $uids) . ")";
        if ($user_info = $this->fetch_all('users', $where)) {
     ...
     * @return int
     */
    public function setAccountInfos($update_data, $uid)
    {
    ...
    public function check_username_char($user_name)
    {
        ...
        switch(get_setting('username_rule'))
        {
            ...
            case 1:
                ...
                {
                    return Application::lang()->_t('请输入大于 %s 字节的用户名, 允许汉字、字母与数字', ($length_min . ' - ' . $length_max));
                }
            break;
            ...
            case 2:
                ...
                {
                    return Application::lang()->_t('请输入 %s 个字母、数字或下划线', ($length_min . ' - ' . $length_max));
                }
            break;
            ...
            case 3:
                ...
                {
                    return Application::lang()->_t('请输入 %s 个汉字', (ceil($length_min / 2) . ' - ' . floor($length_max / 2)));
                }
            break;
        }
    ...
    public function get_users_list($where, $limit = 10, $attrib = false, $exclude_self = true, $orderby = 'uid DESC')
    {
        ...
        if ($exclude_self)
        {
            if ($where)
            {
                $where = '(' . $where . ') AND uid <> ' . Application::user()->get_info('uid');
            }
            else
            {
                $where = 'uid <> ' . Application::user()->get_info('uid');
            }
        }
    ...
    {
        if ($users_list = Application::cache()->get('user_recommend_' . $uid))
        {
            return $users_list;
        }
        ...
        if ($users_list = $this->get_users_list('uid IN(' . implode($friend_uids, ',') . ') AND uid NOT IN (' . implode($follow_uids, ',') . ')', $limit, true, true))
        {
            ...
            Application::cache()->set('user_recommend_' . $uid, $users_list, get_setting('cache_level_normal'));
        }
    ...
    public function delete_avatar($uid)
    {
        ...
        return $this->setAccountInfos(array('avatar_file' => ''), $uid);
    }
    ...
    public function get_user_group_by_id($group_id, $field = null)
    {
        ...
        if (!$user_group = Application::cache()->get('user_group_' . intval($group_id)))
        {
            ...
            Application::cache()->set('user_group_' . intval($group_id), $user_group, get_setting('cache_level_normal'), 'users_group');
        }
    ...
    {
        if (!$user_info = $this->getUserById($uid) OR !$user_group = $this->get_user_group($user_info['group_id']))
        {
            return false;
        }
        ...
        {
            return $this->setAccountInfos(array(
    ...
    public function forbidden_user_by_uid($uid, $status, $admin_uid)
    {
        ...
        return $this->model('account')->setAccountInfos(array(
    ...
    {
        $delete_message = Application::lang()->_t('你发表的内容 %s 已被管理员删除', $title);
        $delete_message .= "\r\n----- " . Application::lang()->_t('内容') . " -----\r\n" . $message;
        $delete_message .= "\r\n-----------------------------\r\n";
        $delete_message .= Application::lang()->_t('如有疑问, 请联系管理员');
    ...
    public function markRecentTopicTitles($uid, $topic_title)
    {
        ...
        $new_recent_topics = array_unique(array_merge(array($topic_title), $recent_topics));

        if (count($new_recent_topics) > 10) {
            $new_recent_topics = array_slice($new_recent_topics, 0, 10);
        }

        return $this->setAccountInfos(array('recent_topics' => serialize($new_recent_topics)), $uid);
    }
