...
class ucenterModel extends Model
{
    ...
    function setup()
    {
        ...
        $this->uc_client_path = realpath(INC_PATH . '../') . '/uc_client/';
    ...
    function login($_username, $_password)
    {
        ...
        if ($uc_uid > 0)
        {
            ...
            else
            {
                ...
                if ($new_user_id = $this->model('account')->user_register($username, $_password, $email, TRUE))
                {
                    ...
                    $user_info = $this->model('account')->getUserById($new_user_id, true, false);
                }
            }
        }
        ...
        else
        {
            if ($user_info['avatar_file'] AND get_setting('ucenter_path'))
            {
                ...
                foreach(Application::config()->get('image')->uc_avatar_thumbnail AS $key => $val)
                {
    ...
    function get_uc_user_info($uc_uid)
    {
        ...
        return $this->model('account')->getUserById($uc_user['uid'], true);
    }
