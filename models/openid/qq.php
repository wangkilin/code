...
class openid_qqModel extends Model
{
    ...
    public function oauth2_login()
    {
        if (!$this->get_access_token() OR !$this->get_openid() OR !$this->get_user_info())
        {
            if (!$this->error_msg)
            {
                $this->error_msg = Application::lang()->_t('QQ 登录失败');
            }
    ...
    public function get_access_token()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 access token 时，与 QQ 通信失败');
    ...
    public function get_openid()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 openid 时，与 QQ 通信失败');
        ...
        if ($result['error'])
        {
            $this->error_msg = Application::lang()->_t('获取 openid 失败，错误为：%s', $result['error_description']);
    ...
    public function get_user_info()
    {
        if (!$this->openid)
        {
            $this->error_msg = Application::lang()->_t('openid 为空');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取个人资料时，与 QQ 通信失败');
        ...
        if ($result['ret'])
        {
            $this->error_msg = Application::lang()->_t('获取个人资料失败，错误为：%s', $result['msg']);
    ...
    public function refresh_access_token($openid)
    {
        ...
        if (!$user_info)
        {
            $this->error_msg = Application::lang()->_t('QQ 账号未绑定');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('更新 access token 时，与 QQ 通信失败');
