...
class openid_weixin_weixinModel extends Model
{
    ...
    public function refresh_access_token($app_id, $app_secret)
    {
        ...
        Application::cache()->delete($cached_token);
    ...
    public function get_access_token($app_id, $app_secret)
    {
        ...
        $access_token = Application::cache()->get($cached_token);
        ...
        Application::cache()->set($cached_token, $result['access_token'], 3600);
    ...
    public function get_sns_access_token_by_authorization_code($code)
    {
        ...
        if (Application::cache()->get($cache_process_key))
        {
            return $this->get_sns_access_token_by_authorization_code($code);
        }

        $sns_access_token = Application::cache()->get($cache_key);
        ...
        Application::cache()->set($cache_process_key, time(), 60);
        ...
        {
            Application::cache()->set($cache_key, $sns_access_token, 60);
        }

        Application::cache()->delete($cache_process_key);
    ...
    public function bind_account($access_user, $access_token, $uid, $is_ajax = false)
    {
        if (! $access_user['nickname'])
        {
            if ($is_ajax)
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('与微信通信出错, 请重新登录')));
            }
            else
            {
                H::redirect_msg(Application::lang()->_t('与微信通信出错, 请重新登录'));
            }
        }

        if ($openid_info = $this->getUserById($uid))
        {
            if ($openid_info['opendid'] != $access_user['openid'])
            {
                if ($is_ajax)
                {
                    H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('微信账号已经被其他账号绑定')));
                }
                else
                {
                    H::redirect_msg(Application::lang()->_t('微信账号已经被其他账号绑定'));
                }
            }
    ...
    public function associate_avatar($uid, $headimgurl)
    {
        if ($headimgurl)
        {
            if (!$user_info = $this->model('account')->getUserById($uid))
            {
                return false;
            }
    ...
    public function register_user($access_token, $access_user)
    {
        ...
        if ($uid = $this->model('account')->user_register($access_user['nickname'], md5(rand(111111, 999999999))))
        {
            ...
            return $this->model('account')->getUserById($uid);
        }
    }
    ...
    public function weixin_login_process($session_id)
    {
        ...
        if ($weixin_login['uid'])
        {
            ...
            return $this->model('account')->getUserById($weixin_login['uid']);
        }
    }
    ...
    public function upload_file($file, $type)
    {
        ...
        $cached_result = Application::cache()->get('weixin_upload_file_' . $file_md5);
        ...
        {
            Application::cache()->set('weixin_upload_file_' . $file_md5, $result, 259200);
        }
    ...
    public function get_file($media_id)
    {
        ...
        $cached_file = Application::cache()->get('weixin_get_file_' . $media_id_md5);
        ...
        if ($file)
        {
            ...
            Application::cache()->set('weixin_get_file_' . $media_id_md5, $file, 259200);
    ...
    public function get_jsapi_ticket($access_token)
    {
        ...
        $jsapi_ticket = Application::cache()->get($jsapi_ticket);
        ...
        Application::cache()->set($cached_ticket, $result['ticket'], 3600);
