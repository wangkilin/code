...
class openid_googleModel extends Model
{
    ...
    public function oauth2_login()
    {
        if (!$this->get_access_token() OR !$this->validate_access_token() OR !$this->get_user_info())
        {
            if (!$this->error_msg)
            {
                $this->error_msg = Application::lang()->_t('Google 登录失败');
            }
    ...
    public function get_access_token()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 access token 时，与 Google 通信失败');
        ...
        if ($result['error'])
        {
            ...
            $this->error_msg = Application::lang()->_t('获取 access token 失败，错误为：%s', $result['error_description']);
    ...
    public function validate_access_token()
    {
        ...
        {
            $this->error_msg = Application::lang()->_t('access token 为空');
        }
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('验证 access token 时，与 Google 通信失败');
        }
        ...
        if ($result['error_description'])
        {
            $this->error_msg = Application::lang()->_t('验证 access token 失败，错误为：%s', $result['error_description']);
        }
    ...
    public function get_user_info()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取个人资料时，与 Google 通信失败');
    ...
    public function refresh_access_token($uid)
    {
        ...
        if (!$user_info)
        {
            $this->error_msg = Application::lang()->_t('Google 账号未绑定');
        ...
        {
            $this->error_msg = Application::lang()->_t('refresh token 为空');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('更新 access token 时，与 Google 通信失败');
