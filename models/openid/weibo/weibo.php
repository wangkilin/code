...
class openid_weibo_weiboModel extends Model
{
    ...
    public function del_msg_by_id($id)
    {
        ...
        {
            $attachs = $this->model('publish')->getAttachByAccessKey('weibo_msg', $msg_info['access_key']);
    ...
    public function reply_answer_to_sina($question_id, $comment)
    {
        ...
        $comment .= ' (' . Application::lang()->_t('来自')  . ' ' . get_js_url('/question/' . $question_id) . ' )';
    ...
    public function get_msg_from_sina_crond()
    {
        ...
        $lock_time = Application::cache()->get('weibo_msg_locker');
        ...
        Application::cache()->set('weibo_msg_locker', $now, 600);

        foreach ($services_info AS $service_info)
        {
            $service_user_info = $this->model('account')->getUserById($service_info['uid']);
            ...
            foreach ($result['statuses'] AS $msg)
            {
                ...
                if ($msg['pic_urls'] AND get_setting('upload_enable') == 'Y')
                {
                    foreach ($msg['pic_urls'] AS $pic_url)
                    {
                        ...
                        Application::upload()->do_upload($pic_url_array[3], $result);
        ...
        Application::cache()->delete('weibo_msg_locker');
