...
class openid_weibo_oauthModel extends Model
{
    ...
    public function oauth2_login()
    {
        if (!$this->get_access_token() OR !$this->validate_access_token() OR !$this->get_user_info())
        {
            if (!$this->error_msg)
            {
                $this->error_msg = Application::lang()->_t('微博登录失败');
            }
    ...
    public function get_access_token()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 access token 时，与微博通信失败');
        ...
        {
            $this->error_msg = Application::lang()->_t('获取 access token 失败，错误为：%s', $result['error']);
    ...
    public function validate_access_token()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('验证 access token 时，与微博通信失败');
        }
        ...
        if ($result['error_code'])
        {
            $this->error_msg = Application::lang()->_t('验证 access token 失败，错误为：%s', $result['error']);
    ...
    public function get_user_info()
    {
        ...
        if (!$this->uid)
        {
            $this->error_msg = Application::lang()->_t('uid 为空');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取个人资料时，与微博通信失败');
        ...
        if ($result['error_code'])
        {
            $this->error_msg = Application::lang()->_t('获取个人资料失败，错误为：%s', $result['error']);
