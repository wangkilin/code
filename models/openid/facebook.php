...
class openid_facebookModel extends Model
{
    ...
    public function oauth2_login()
    {
            ...
            OR !$this->validate_user_access_token() OR !$this->get_user_info())
        {
            if (!$this->error_msg)
            {
                $this->error_msg = Application::lang()->_t('Facebook 登录失败');
            }
    ...
    public function get_user_access_token()
    {
        ...
        {
            $this->error_msg = Application::lang()->_t('authorization code 为空');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 user access token 时，与 Facebook 通信失败');
        ...
        if (!$user_access_token['access_token'])
        {
            ...
            $this->error_msg = ($result['error']) ? Application::lang()->_t('获取 user access token 失败，错误为：%s', $result['error']['message'])
                : Application::lang()->_t('获取 user access token 失败');
    ...
    public function get_app_access_token()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取 app access token 时，与 Facebook 通信失败');
        ...
        if (!$app_access_token['access_token'])
        {
            ...
            $this->error_msg = ($result['error']) ? Application::lang()->_t('获取 app access token 失败，错误为：%s', $result['error']['message'])
                : Application::lang()->_t('获取 app access token 失败');
    ...
    public function validate_user_access_token()
    {
        ...
        {
            $this->error_msg = Application::lang()->_t('user access token 为空');
        }

        if (!$this->app_access_token)
        {
            $this->error_msg = Application::lang()->_t('app access token 为空');
        }
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('验证 user access token 时，与 Facebook 通信失败');
        }
        ...
        if ($result['error'])
        {
            $this->error_msg = Application::lang()->_t('验证 user access token 失败，错误为：%s', $result['error']['message']);
        ...
        {
            $this->error_msg = Application::lang()->_t('验证 user access token 失败，错误为：%s', $result['data']['error']['message']);
    ...
    public function get_user_info()
    {
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('获取个人资料时，与 Facebook 通信失败');
        ...
        if ($result['error'])
        {
            $this->error_msg = Application::lang()->_t('获取个人资料失败，错误为：%s', $result['error']['message']);
    ...
    public function refresh_access_token($uid)
    {
        ...
        if (!$user_info)
        {
            $this->error_msg = Application::lang()->_t('Facebook 账号未绑定');
        ...
        if (!$result)
        {
            $this->error_msg = Application::lang()->_t('更新 access token 时，与 Facebook 通信失败');
        ...
        if ($result['error'])
        {
            ...
            $this->error_msg = Application::lang()->_t('更新 access token 失败，错误为：%s', $result['error_description']);
