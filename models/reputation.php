...
class reputationModel extends Model
{
    ...
    public function calculate_by_uid($uid)
    {
        ...
        if ($user_articles = $this->query_all('SELECT id FROM ' . get_table('article') . '
                WHERE uid = ' . $user_info['uid'])) {
            foreach ($user_articles as $articles_val) {
                $articles_ids[] = $articles_val['id'];
            }

            if ($articles_ids) {
                $articles_vote_agree_users = $this->model('article')->getVoteByArticleIds('article', $articles_ids, 1);
            ...
            // 计算在话题中的威望
            if ($article_reputation && $article_topics[$articles_val['id']]) {
        ...
        if ($users_anwsers = $this->query_all('SELECT answer_id, question_id, agree_count, thanks_count
                FROM ' . get_table('answer') . '
                WHERE uid = ' . $user_info['uid'])) {
            foreach ($users_anwsers as $answers_val) {
                $answer_ids[] = $answers_val['answer_id'];
                $question_ids[] = $answers_val['question_id'];
            }

            if ($question_ids) {
                if ($questions_info_query = $this->query_all('SELECT question_id, best_answer, published_uid, category_id
                        FROM ' . get_table('question') . '
                        WHERE question_id IN(' . implode(',', $question_ids) . ')')) {
                    foreach ($questions_info_query AS $questions_info_val) {
                        $questions_info[$questions_info_val['question_id']] = $questions_info_val;
                    }
                    unset($questions_info_query);
                }

                if ($question_topics_query = $this->query_all('SELECT item_id, topic_id
                        FROM ' . get_table('topic_relation') . '
                        WHERE item_id IN(' . implode(',', $question_ids) . ")
                          AND `type` = 'question'")) {
            ...
            foreach ($users_anwsers as $answers_key => $answers_val) {
                ...
                if ($vote_agree_users[$answers_val['answer_id']]) {
                    foreach ($vote_agree_users[$answers_val['answer_id']] AS $key => $val) {
                        ...
                        if ($questions_info[$answers_val['question_id']]['published_uid'] == $val['vote_uid']
                                AND !$s_publisher_agree) {
                            $s_publisher_agree = 1;
                        }
                    }
                }
                ...
                // 计算在话题中的威望
                if ($answer_reputation && $question_topics[$answers_val['question_id']]) {
                    foreach ($question_topics[$answers_val['question_id']] as $topic_info) {
                        $user_topics[$topic_info['topic_id']] = array(
                            'topic_id' => $topic_info['topic_id'],
                            'count' => (intval($user_topics[$topic_info['topic_id']]['count']) + 1),
        ...
        if (is_array($user_topics)) {
            ...
            foreach ($user_topics as $t_key => $t_val) {
                ...
                {
                    $this->update('reputation_topic', $data, 'auto_id = ' . $reputation_topic_id);
                } else {
                    $this->insert('reputation_topic', $data);
                }
            }
        }
        ...
        if (is_array($user_reputation_category)) {
            foreach ($user_reputation_category as $t_key => $t_val) {
                ...
                if ($user_reputation_category_id = $this->fetch_one('reputation_category', 'auto_id', 'uid = ' . intval($uid) . ' AND category_id = ' . $t_key)) {
                    $this->update('reputation_category', $data, 'auto_id = ' . $user_reputation_category_id);
                } else {
                    $this->insert('reputation_category', $data);
                }
            }
        }
