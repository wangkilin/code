...
class emailModel extends Model
{
	...
	*/
	...
	public function action_email($action, $email, $link, $data = array(), $server = 'master')
	{
		...
		{
			$user_info = $this->model('account')->getUserById($email);
		...
		$email_message = (array)Application::config()->get('email_message');
	...
	public function get_mail_template($user_name, $subject, $message, $link = null, $link_title = null)
	{
		...
		View::assign('link_title', $link_title);

		return View::output('global/email_template', false);
	}
	...
	public function send($email, $subject, $message, $link = null, $link_title = null, $server = 'master')
	{
		if (is_digits($email))
		{
			if (! $user_info = $this->model('account')->getUserById($email))
			{
				return false;
			}
		...
		return Application::mail()->send($email, $subject, $this->get_mail_template($user_name, $subject, $message, $link, $link_title), get_setting('site_name'), $user_name, $server);
	}
	...
	public function send_mail_queue($limit = 10)
	{
		if ($mail_queue = $this->fetch_all('mail_queue', 'is_error = 0', 'id ASC', $limit))
		{
			...
			{
				if ($error_message = Application::mail()->send($val['send_to'], $val['subject'], $val['message'], get_setting('site_name'), null, 'master'))
				{
