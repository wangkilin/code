...
class peopleModel extends Model
{
    ...
    {
        if (Application::cache()->get('update_views_people_' . md5(session_id()) . '_' . intval($uid)))
        {
            return false;
        }

        Application::cache()->set('update_views_people_' . md5(session_id()) . '_' . intval($uid), time(), get_setting('cache_level_normal'));
    ...
    public function get_users_reputation_topic($uids, $users_reputation, $limit = 10)
    {
        ...
        if ($reputation_topics)
        {
            ...
            $topics = $this->model('topic')->getTopicsByIds($topic_ids);
    ...
    public function get_near_by_users($longitude, $latitude, $uid, $limit = 10)
    {
        ...
        if ($near_by_uids)
        {
            if ($near_by_users = $this->model('account')->getUsersByIds($near_by_uids))
            {
