...
class notifyModel extends Model
{
    ...
    {
        if ($this->notify_action_details = Application::config()->get('notification')->action_details)
        {
    ...
    public function list_notification($recipient_uid, $read_status = 0, $limit = null)
    {
        ...
        if ($question_ids)
        {
            $question_list = $this->model('question')->getQuestionsByIds($question_ids);
        }
        ...
        if ($uids)
        {
            $user_infos = $this->model('account')->getUsersByIds($uids);
        }
        ...
        foreach ($notify_list as $key => $notify)
        {
            ...
            if ($data['from_uid'])
            {
                ...
                $tmp_data['p_url'] = get_js_url('/user/' . $user_info['url_token']);
            }
        ...
        foreach ($extends as $action_type => $val)
        {
            ...
            foreach ($action_users as $uid => $action)
            {
                ...
                $tmp_data['users'][$uid] = array(
                    'username' => $anonymous ? Application::lang()->_t('匿名用户') : $user_infos[$uid]['user_name'],
                    'url' => $url
                );
            }
    ...
    public function format_notification($data)
    {
        foreach ($data AS $key => $val)
        {
            if ($val['extend_count'] > 1)
            {
                ...
                {
                    case self::CATEGORY_QUESTION:
                        $data[$key]['message'] = $val['extend_count'] . ' ' . Application::lang()->_t('项关于问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::CATEGORY_ARTICLE:
                        $data[$key]['message'] = $val['extend_count'] . ' ' . Application::lang()->_t('项关于文章') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
                ...
                foreach($val['extend_details'] AS $action_type => $extend)
                {
                    ...
                    switch ($action_type)
                    {
                        case self::TYPE_ANSWER_AT_ME:
                            $data[$key]['extend_message'][] = Application::lang()->_t('他们在回答中提到了你') . ': ' . $users_list;
                            break;

                        case self::TYPE_COMMENT_AT_ME:
                            $data[$key]['extend_message'][] = Application::lang()->_t('他们在问题中的评论提到了你') . ': ' . $users_list;

                            break;

                        case self::TYPE_ANSWER_COMMENT_AT_ME:
                            $data[$key]['extend_message'][] = Application::lang()->_t('他们在回答中的评论提到了你') . ': ' . $users_list;

                            break;

                        case self::TYPE_ARTICLE_COMMENT_AT_ME:
                            $data[$key]['extend_message'][] = Application::lang()->_t('他们在文章中的评论提到了你') . ': ' . $users_list;

                            break;

                        case self::TYPE_ARTICLE_NEW_COMMENT:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个新回复, 按评论人查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_NEW_ANSWER:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个新回复, 按回答人查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_ANSWER_COMMENT:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个新评论, 按评论人查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_ANSWER_AGREE:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个新赞同, 按赞同者查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_ANSWER_THANK:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个新感谢, 按感谢者查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_MOD_QUESTION:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 次编辑问题, 按编辑者查看', $extend['count']) . ': ' . $users_list;

                            break;

                        case self::TYPE_REDIRECT_QUESTION:
                            $data[$key]['extend_message'][] = $users_list . ' ' . Application::lang()->_t('重定向了你发布的问题');

                            break;

                        case self::TYPE_REMOVE_ANSWER:
                            $data[$key]['extend_message'][] = Application::lang()->_t('%s 个回复被删除', $extend['count']);

                            break;

                        case self::TYPE_INVITE_QUESTION:
                            $data[$key]['extend_message'][] = $users_list . ' ' . Application::lang()->_t('邀请你参与问题');
            ...
            else
            {
                switch ($val['action_type'])
                {
                    case self::TYPE_PEOPLE_FOCUS:
                        $data[$key]['message'] = '<a href="' . $val['key_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('关注了你');
                    ...
                    case self::TYPE_NEW_ANSWER:
                        if ($val['anonymous'])
                        {
                            $data[$key]['message'] = Application::lang()->_t('匿名用户');
                        }
                        else
                        {
                        ...
                        $data[$key]['message'] .= ' ' . Application::lang()->_t('回复了问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_ARTICLE_NEW_COMMENT:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('评论了文章') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_COMMENT_AT_ME:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的评论提到了你');

                        break;

                    case self::TYPE_ARTICLE_COMMENT_AT_ME:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('在文章') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('评论中回复了你');
                    ...
                    case self::TYPE_ANSWER_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . Application::lang()->_t('在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的回答提到了你');
                    ...
                    case self::TYPE_ANSWER_COMMENT_AT_ME:
                        ...
                        $data[$key]['message'] .= ' ' . Application::lang()->_t('在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('回答评论中提到了你');
                    break;

                    case self::TYPE_INVITE_QUESTION:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('邀请你参与问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_ANSWER_COMMENT:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('评论了你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的回复');

                        break;

                    case self::TYPE_QUESTION_COMMENT:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('评论了你发起的问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_ANSWER_AGREE:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('赞同了你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的回复');

                        break;

                    case self::TYPE_ANSWER_THANK:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('感谢了你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的回复');

                        break;

                    case self::TYPE_MOD_QUESTION:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('编辑了你发布的问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_REMOVE_ANSWER:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('删除了你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . Application::lang()->_t('中的回复');

                        break;

                    case self::TYPE_REDIRECT_QUESTION:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('重定向了你发起的问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';

                        break;

                    case self::TYPE_QUESTION_THANK:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . Application::lang()->_t('感谢了你发起的问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
                    ...
                    case self::TYPE_ARTICLE_APPROVED:
                        $data[$key]['message'] = Application::lang()->_t('你发起的文章 %s 审核通过', '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>');

                        break;

                    case self::TYPE_ARTICLE_REFUSED:
                        $data[$key]['message'] = Application::lang()->_t('你发起的文章 %s 审核未通过', $val['title']);

                        break;

                    case self::TYPE_QUESTION_APPROVED:
                        $data[$key]['message'] = Application::lang()->_t('你发起的问题 %s 审核通过', '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>');

                        break;

                    case self::TYPE_QUESTION_REFUSED:
                        $data[$key]['message'] = Application::lang()->_t('你发起的问题 %s 审核未通过', $val['title']);

                        break;

                    case self::TYPE_TICKET_CLOSED:
                        $data[$key]['message'] = Application::lang()->_t('%s0 关闭了你发起的工单 %s1', array(
                    ...
                    case self::TYPE_TICKET_REPLIED:
                        $data[$key]['message'] = Application::lang()->_t('%s0 回复了你发起的工单 %s1', array(
                            '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ',
                            '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
                        ));
