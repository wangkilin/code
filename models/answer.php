...
class answerModel extends Model
{
	...
	public function getAnswersByIds($answer_ids)
	{
	...
	public function get_answer_list_by_question_id($question_id, $limit = 20, $where = null, $order = 'answer_id DESC')
	{
		...
		if ($uids)
		{
			if ($users_info = $this->model('account')->getUsersByIds($uids, true))
			{
				foreach($answer_list as $key => $val)
				{
	 ...
	 */
	public function getVoteUsersByAnswerId($answer_id)
	{
		$list = array();
		if (!$answer_id) {
			return $list;
		}
		$users = $this->getVoteUsersByAnswerIds(array($answer_id));
		if ($users) {
			$list = end($users);
		}
		...
		if ($users = $this->query_all("SELECT vote_uid FROM " . $this->get_table('answer_vote') . " WHERE answer_id = " . intval($answer_id) . " AND vote_value = 1"))
		{
			...
			$vote_users_info = $this->model('account')->getUsersByIds($vote_users_ids);
	 ...
	 * @return multitype:|unknown
	 */
	public function getVoteUsersByAnswerIds($answer_ids)
	{
		if (!is_array($answer_ids)) {
			return array();
		}
		...
		if ($users = $this->query_all("SELECT vote_uid, answer_id
				FROM " . $this->get_table('answer_vote') . "
				WHERE answer_id IN(" . implode(',', $answer_ids) . ")
				  AND vote_value = 1"))  {
			foreach ($users as $key => $val) {
				$vote_users_ids[] = $val['vote_uid'];
			}
			...
			foreach ($users as $key => $val) {
				if (! isset($data[$val['answer_id']])) {
					$data[$val['answer_id']] = array();
				}
				// 绑定到答案id和用户id
	...
	public function change_answer_vote($answer_id, $vote_value = 1, $uid = 0,$reputation_factor=0)
	{
		...
		if (!$vote_info = $this->checkUserVoteAnswerStatus($answer_id, $uid)) //添加记录
		{
		...
		{
			$this->model('notify')->send($uid, $answer_info['uid'], notifyModel::TYPE_ANSWER_AGREE, notifyModel::CATEGORY_QUESTION, $question_id, array(
				'from_uid' => $uid,
	...
	/**
	 * 获取用户在答案上的投票情况
	 * @param int|array $answer_id 答案id，ids
	 * @param unknown $uid
	 * @return string|multitype:
	 */
	public function checkUserVoteAnswerStatus($answer_id, $uid)
	{
		if (is_array($answer_id)) {
			array_walk_recursive($answer_id, 'intval_string');

			if ($result = $this->query_all("SELECT answer_id, vote_value
					FROM " . get_table('answer_vote') . "
					WHERE answer_id IN(" . implode(',', $answer_id) . ")
					  AND vote_uid = " . intval($uid))) {
	...
	public function remove_answer_by_id($answer_id)
	{
		if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
		{
			...
			if ($attachs = $this->model('publish')->getAttachListByItemTypeAndId('answer', $answer_id))
			{
	...
	public function get_answer_agree_users($answer_id)
	{
		...
		if ($users = $this->model('account')->getUsersByIds($agree_uids))
		{
			foreach ($users as $key => $val)
			{
	...
	public function insert_answer_comment($answer_id, $uid, $message)
	{
		...
		{
			$this->model('notify')->send($uid, $answer_info['uid'], notifyModel::TYPE_ANSWER_COMMENT, notifyModel::CATEGORY_QUESTION, $answer_info['question_id'], array(
				'from_uid' => $uid,
				'question_id' => $answer_info['question_id'],
			...
			if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($answer_info['uid']))
			{
		...
		if ($at_users = $this->model('question')->parse_at_user($message, false, true))
		{
			foreach ($at_users as $user_id)
			{
				if ($user_id != $uid)
				{
					$this->model('notify')->send($uid, $user_id, notifyModel::TYPE_ANSWER_COMMENT_AT_ME, notifyModel::CATEGORY_QUESTION, $answer_info['question_id'], array(
						'from_uid' => $uid,
						'question_id' => $answer_info['question_id'],
						'item_id' => $answer_info['answer_id'],
						'comment_id' => $comment_id
					...
					{
						$answer_user = $this->model('account')->getUserById($uid);
