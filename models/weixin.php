...
class weixinModel extends Model
{
    ...
    public function response_message($input_message)
    {
        switch ($input_message['msgType'])
        {
            ...
            case 'location':
                if ($near_by_questions = $this->model('people')->get_near_by_users($input_message['location_X'], $input_message['location_Y'], $this->user_id, 5))
                {
                    foreach ($near_by_questions AS $key => $val)
                    {
                        if (!$response_message)
                        {
                            $image_file = Application::config()->get('weixin')->default_list_image;
                        }
                        else
                        {
            ...
            case 'image':
                ...
                {
                    Application::cache()->set('weixin_pic_url_' . md5($input_message['mediaID']), $input_message['picUrl'], 259200);
    ...
    public function message_parser($input_message, $param = null)
    {
        ...
        switch ($message_code)
        {
            default:
                if (cjk_strlen($input_message['content']) > 1 AND substr($input_message['content'], 0, 1) == '@')
                {
                    if ($user_info = $this->model('account')->get_user_info_by_username(substr($input_message['content'], 1), true))
                    {
                        $response_message[] = array(
                            ...
                            'link' => get_js_url('/m/user/' . $user_info['url_token']),
                ...
                else if ($topic_info = $this->model('topic')->getTopicByTitle($input_message['content']))
                {
                    $response_message[] = array(
    ...
    {
        if (!$characteristic = Application::config()->get('weixin')->language_characteristic[$type])
        {
            return false;
        }
    ...
    public function update_client_menu($account_info)
    {
        ...
        if (!$result)
        {
            return Application::lang()->_t('远程服务器忙,请稍后再试');
        }
    ...
    {
        $groups = Application::cache()->get('weixin_groups');
        ...
        if (!$groups)
        {
            ...
            if (!$result)
            {
                return Application::lang()->_t('远程服务器忙');
            }
            ...
            Application::cache()->set('weixin_groups', $groups, get_setting('cache_level_normal'));
        }
    ...
    public function add_articles_to_mpnews($article_ids)
    {
        ...
        $users_info = $this->model('account')->getUsersByIds($published_uids);
    ...
    {
        $questions_info = $this->model('question')->getQuestionsByIds($question_ids);
    ...
    public function upload_mpnews()
    {
        if (!$this->mpnews)
        {
            return Application::lang()->_t('没有要群发的内容');
        }
    ...
    public function create_qr_code($scene_id)
    {
        if (!$scene_id)
        {
            return Application::lang()->_t('scene_id 错误');
        }
        ...
        if (!$result['ticket'])
        {
            ...
            return Application::lang()->_t('获取 ticket 失败');
        }
        ...
        if (!$qr_code)
        {
            ...
            return Application::lang()->_t('换取二维码失败');
        }
        ...
        Application::upload()->do_upload($img_file_name, $qr_code);
        ...
        if ($upload_error)
        {
            ...
            return Application::lang()->_t('保存二维码图片失败，错误为 %s', $upload_error);
        }
        ...
        if (!$upload_data)
        {
            ...
            return Application::lang()->_t('保存二维码图片失败，请与管理员联系');
        }
    }
