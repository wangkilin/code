...
class edmModel extends Model
{
    ...
    public function run_task()
    {
        ...
        foreach ($user_list AS $key => $item)
        {
            ...
            Application::mail()->send($item['email'], $task_data[$item['taskid']]['subject'], $message, $from_name, null, 'slave');
        }
    ...
    public function receive_email_crond()
    {
        ...
        $lock_time = Application::cache()->get('receive_email_locker');
        ...
        Application::cache()->set('receive_email_locker', $now, 600);
        ...
        Application::cache()->delete('receive_email_locker');
    ...
    public function reply_answer_by_email($question_id, $comment)
    {
        ...
        $comment .= ' <br /> <a href="' . get_js_url('/question/' . $question_id) . ' target="_blank">' . Application::lang()->_t('查看问题详情') . '</a>';

        return Application::mail()->send($received_email['from'], 'RE: ' . $received_email['subject'], $comment, get_setting('site_name'));
    }
