...
class questionModel extends Model
{
    ...
    /**
     * 通过问起ids获取问题。 返回数组索引为问题的id
     * @param array $question_ids
     * @return boolean|unknown
     */
    public function getQuestionsByIds($question_ids)
    {
        if (!$question_ids) {
            return false;
        }
    ...
    {
        if (Application::cache()->get('update_views_question_' . md5(session_id()) . '_' . intval($question_id)))
        {
            return false;
        }

        Application::cache()->set('update_views_question_' . md5(session_id()) . '_' . intval($question_id), time(), 60);
    ...
    public function update_question($question_id, $question_content, $question_detail,
            $uid, $verified = true, $modify_reason = null,
            $anonymous = null, $category_id = null, $type=null, $type_id = 0)
    {
    ...
    public function remove_question($question_id)
    {
        ...
        // 删除附件
        if ($attachs = $this->model('publish')->getAttachListByItemTypeAndId('question', $question_id))
        {
     ...
     */
    public function getUserFollowQuestionsStatus($question_ids, $uid)
    {
        $followStatus = array();
        ...
        if ($question_focus) {
            foreach ($question_focus AS $key => $val) {
                $followStatus[$val['question_id']] = $val['focus_id'];
            }
        }
    ...
    public function get_focus_users_by_question($question_id, $limit = 10)
    {
        ...
        {
            $users_list = $this->model('account')->getUsersByIds(fetch_array_value($uids, 'uid'));
        }
    ...
    public function get_related_question_list($question_id, $question_content, $limit = 10)
    {
        ...
        if ($question_related_list = Application::cache()->get($cache_key))
        {
        ...
        Application::cache()->set($cache_key, $question_related_list, get_setting('cache_level_low'));
    ...
    {
        if ($invite_users_list = Application::cache()->get('question_invite_users_' . $question_id))
        {
        ...
        if ($invites = $this->fetch_all('question_invite', 'question_id = ' . intval($question_id), 'question_invite_id DESC', $limit))
        {
            ...
            $invite_users_list = $this->model('account')->getUsersByIds($invite_users);

            Application::cache()->set('question_invite_users_' . $question_id, $invite_users_list, get_setting('cache_level_normal'));
        }
    ...
    public function get_invite_question_list($uid, $limit = 10)
    {
        if ($list = $this->fetch_all('question_invite', 'recipients_uid = ' . intval($uid), 'question_invite_id DESC', $limit))
        {
            ...
            $question_infos = $this->getQuestionsByIds($question_ids);
    ...
    public function parse_at_user($content, $popup = false, $with_user = false, $to_uid = false)
    {
        ...
        if (is_array($matchs[1]))
        {
            ...
            foreach ($match_name as $key => $user_name)
            {
                ...
                {
                    $user_info = $this->model('account')->getUserById($user_name);
                }
                else
                {
                ...
                if ($user_info)
                {
                    $content = str_replace('@' . $user_name, '<a href="user/' . $user_info['url_token'] . '"' . (($popup) ? ' target="_blank"' : '') . ' class="icb-user-name" data-id="' . $user_info['uid'] . '">@' . $user_info['user_name'] . '</a>', $content);
    ...
    public function insert_question_comment($question_id, $uid, $message)
    {
        ...
        {
            $this->model('notify')->send($uid, $question_info['published_uid'], notifyModel::TYPE_QUESTION_COMMENT, notifyModel::CATEGORY_QUESTION, $question_info['question_id'], array(
                'from_uid' => $uid,
                'question_id' => $question_info['question_id'],
                'comment_id' => $comment_id
            ));

            if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($question_info['published_uid']))
            {
        ...
        if ($at_users = $this->model('question')->parse_at_user($message, false, true))
        {
            foreach ($at_users as $user_id)
            {
                ...
                $this->model('notify')->send($uid, $user_id, notifyModel::TYPE_COMMENT_AT_ME, notifyModel::CATEGORY_QUESTION, $question_info['question_id'], array(
                    'from_uid' => $uid,
                    'question_id' => $question_info['question_id'],
                    'comment_id' => $comment_id
                ));

                if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($user_id))
                {
                    $weixin_user_info = $this->model('account')->getUserById($weixin_user['uid']);
    ...
    public function analysis_log($log_list, $published_uid = 0, $anonymous = 0)
    {
        ...
        if ($log_user_ids)
        {
            $log_users_info = $this->model('account')->getUsersByIds($log_user_ids);
        }
        ...
        foreach ($log_list as $key => $log)
        {
            ...
            $user_url = 'user/' . $user_info['url_token'];
            ...
            switch ($log['associate_action'])
            {
                ...
                case ACTION_LOG::ADD_TOPIC : //添加话题
                    ...
                    $topic_info = $this->model('topic')->getTopicById($log['associate_attached']);
                ...
                case ACTION_LOG::MOD_QUESTION_CATEGORY : //修改分类
                    ...
                    $title_list = $user_name_string . ' 修改了该问题的分类 <p><a href="index/category-' . $log['associate_attached'] . '">' . $log['associate_content'] . '</a>';
    ...
    public function getUserRelatedTopics($question_content)
    {
        if ($question_related_list = $this->get_related_question_list(null, $question_content, 10))
        {
            ...
            {
                if ($topic_info = $this->model('topic')->getTopicById($topic_id))
                {
    ...
    {
        if ($result = Application::cache()->get('answer_users_by_question_id_' . md5($question_id . $limit . $published_uid)))
        {
            return $result;
        }
        ...
        if ($answer_users = $this->query_all("SELECT DISTINCT uid FROM " . get_table('answer') . " WHERE question_id = " . intval($question_id) . " AND uid <> " . intval($published_uid) . " AND anonymous = 0 ORDER BY agree_count DESC LIMIT " . intval($limit)))
        {
            ...
            $result = $this->model('account')->getUsersByIds($answer_uids);

            Application::cache()->set('answer_users_by_question_id_' . md5($question_id . $limit . $published_uid), $result, get_setting('cache_level_normal'));
        }
    ...
    public function get_near_by_questions($longitude, $latitude, $uid, $limit = 10)
    {
        ...
        if ($near_by_locations = $this->fetch_all('geo_location', "item_type = 'question' AND `latitude` > 0 AND `latitude` > " . $squares['BR']['latitude'] . " AND `latitude` < " . $squares['TL']['latitude'] . " AND `longitude` > " . $squares['TL']['longitude'] . " AND `longitude` < " . $squares['BR']['longitude'], 'add_time DESC', null, $limit))
        {
            ...
            if ($near_by_questions = $this->getQuestionsByIds($near_by_question_ids))
            {
                ...
                $near_by_users = $this->model('account')->getUsersByIds($near_by_uids);
