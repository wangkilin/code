...
class integralModel extends Model
{
	...
	public function parse_log_item($parse_items)
	{
		...
		if ($answer_ids)
		{
			$answers_info = $this->model('answer')->getAnswersByIds($answer_ids);
		}

		if ($user_ids)
		{
			$users_info = $this->model('account')->getUsersByIds($user_ids);
		}
				...
				case 'INVITE':
					if ($users_info[$item['item_id']])
					{
						$result[$log_id] = array(
							...
							'url' => get_js_url('/user/' . $users_info[$item['item_id']]['uid'])
						);
					}
				break;
			}
