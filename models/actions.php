...
class actionsModel extends Model
{
    public function home_activity($uid, $limit = 10)
    {
        ...
        {
            if ($user_focus_topics_questions_ids = $this->model('topic')->getItemIdsByTopicIds($user_focus_topics_ids, 'question', 1000))
            {
            ...
            $user_focus_topics_article_ids = $this->model('topic')->getItemIdsByTopicIds($user_focus_topics_ids, 'article', 1000);
        }
        ...
        {
            $question_infos = $this->model('question')->getQuestionsByIds($action_list_question_ids);
        ...
        {
            $answer_infos = $this->model('answer')->getAnswersByIds($action_list_answer_ids);

            foreach ($answer_infos as $key => $val) {
                $answer_infos[$key]['answer_content'] = strip_ubb($val['answer_content']);
            }

            $answer_attachs = $this->model('publish')->getAttachesByItemTypeAndIds('answer', $action_list_answer_ids, 'min');
        }
        ...
        if ($action_list_uids)
        {
            $user_info_lists = $this->model('account')->getUsersByIds($action_list_uids, true);
        }
        ...
        {
            $answer_agree_users = $this->model('answer')->getVoteUsersByAnswerIds($final_list_answer_ids);
            $answer_vote_status = $this->model('answer')->checkUserVoteAnswerStatus($final_list_answer_ids, $uid);
    ...
    public function get_user_actions($uid, $limit = 10, $actions = false, $this_uid = 0)
    {
        ...
        if ($user_actions = Application::cache()->get($cache_key))
        {
            return $user_actions;
        }
        ...
        if ($uids)
        {
            $action_list_users = $this->model('account')->getUsersByIds($uids, true);
        }

        if ($question_ids)
        {
            $action_questions_info = $this->model('question')->getQuestionsByIds($question_ids);
        }

        if ($associate_topic_ids)
        {
            $associate_topics = $this->model('topic')->getTopicsByIds($associate_topic_ids);
        }
        ...
        Application::cache()->set($cache_key, $action_list, get_setting('cache_level_normal'));
