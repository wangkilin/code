...
class Db_Pdo extends Db_Abstract
{
	...
	public function __construct($config)
	{
		try {
			$initQuery = array();
			$dsn = '';
			if (! isset($config['options']['databaseType'])) {
				throw new Exception('Database type is not specified in DB config.options');
			}

			$databaseType = strtolower($config['options']['databaseType']);
			$doCharset = true;
			$initQuery = null;
			switch ($databaseType) {
				case 'mariadb':
				case 'mysql':
					if (isset($this->_config['options']['socket'])) {
						$dsn = 'mysql:'
							 . 'unix_socket=' . $this->_config['options']['socket'] . ';'
					...
					$initQuery = 'SET SQL_MODE=ANSI_QUOTES';
					break;
				...
				case 'sybase':
					$dsn = 'dblib:host=' . $this->_config['host'] . ';'
						. ($this->_config['port'] ? 'port=' . $this->_config['port'] : '') . ';'
						. 'dbname=' . $this->_config['database'];
					break;

				case 'oracle':
					$dsn = 'oci:dbname=//' . $this->_config['host'] . ':'
						. ($this->_config['port'] ? $this->_config['port'] : '1521') . '/'
						. 'dbname=' . $this->_config['database'] . ';'
				...
				case 'mssql':
					...
					$initQuery = 'SET QUOTED_IDENTIFIER ON';
					break;

				case 'sqlite':
					$dsn = 'sqlite:' . $this->_config['database'];// sqlite文件名放在配置database中
					$this->username = null;
					$this->password = null;
					$doCharset = false;
					break;
			...
			if ($doCharset && isset($this->_config['charset'])) {
				$this->pdo->exec("SET NAMES '" . $this->_config['charset'] . "'");
			}
	...
	public function query($query)
	{
        ...
        $this->_query = $this->pdo->query($query);
        if (! $this->_query) {
        	$error = $this->getError();
