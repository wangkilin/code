...
class View
{
    ...
    public static function initialize()
    {
        if (!is_object(self::$view))
        {
            ...
            if (file_exists(INC_PATH . 'config.inc.php') AND class_exists('Application', false))
            {
                self::$in_app = true;
            }
        }
    ...
    public static function output($template_filename, $display = true)
    {
        ...
        if (self::$in_app AND basename($template_filename) != 'debuger.php')
        {
            ...
            */
            ...
            if (get_setting('url_rewrite_enable') == 'Y' AND $template_dirs[0] != 'admin')
            {
                ...
                $output = preg_replace('/<([^>]*?)(href|action)=([\"|\'])(?!mailto)(?!file)(?!ftp)(?!http)(?!javascript)(?![\/|\#])(?!\.{1,2}\/)([^\"\']{0,})([\"|\'])([^>]*?)>/is', '<\1\2=\3' . base_url() . '/' . '\4\5\6>', $output);
            }
    ...
    /**
     * 将待引入的js/css列表清空
     * @param string(js,css)|false $type
     */
    ...
    /**
     * 判断模板中是否包含了某另外模板。 如果包含， 需要model获取指定数据
     * @param string $output_filename 是否包含这个模板文件名
     * @param string $template_filename 全局模板文件路径
     * @return boolean
     */
    public static function is_output($output_filename, $template_filename)
    {
        ...
        {
            preg_match_all("/View::output\(['|\"](.+)['|\"]\)/i", self::fetch($template_filename), $matchs);
