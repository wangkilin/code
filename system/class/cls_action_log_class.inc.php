...
class ACTION_LOG
{
	...
	public static function associate_fresh_action($history_id, $associate_id, $associate_type, $associate_action, $uid, $anonymous, $add_time)
	{
		...
		Application::model()->delete('user_action_history_fresh', 'associate_id = ' . intval($associate_id) . ' AND associate_type = ' . intval($associate_type) . ' AND uid = ' . intval($uid));
		...
		{
			// 删除相同关联 ID 下相同动作的旧动态
			Application::model()->delete('user_action_history_fresh', 'associate_id = ' . intval($associate_id) . ' AND associate_type = ' . intval($associate_type) . ' AND associate_action = ' . intval($associate_action));
		}

		return Application::model()->insert('user_action_history_fresh', array(
	...
	public static function save_action($uid, $associate_id, $action_type, $action_id, $action_content = null, $action_attch = null, $add_time = null, $anonymous = null, $addon_data = null)
	{
		...
		$history_id = Application::model()->insert('user_action_history', array(
			'uid' => intval($uid),
		...
		Application::model()->insert('user_action_history_data', array(
	...
	public static function get_action_by_event_id($event_id = 0, $limit = 20, $action_type = null, $action_id = null, $associate_attached = null)
	{
		...
		{
			$where[] = "associate_attached  = '" . Application::model()->quote($associate_attached) . "'";
		}

		if ($user_action_history = Application::model()->fetch_all('user_action_history', implode(' AND ', $where), 'add_time DESC', $limit))
		{
	...
	{
		if ($action_data = Application::model()->fetch_all('user_action_history_data', 'history_id IN(' . implode(',', $history_ids) . ')'))
		{
	...
	{
		return Application::model()->fetch_row('user_action_history_data', 'history_id = ' . intval($history_id));
	}

	public static function get_action_by_history_id($history_id)
	{
		if ($action_history = Application::model()->fetch_row('user_action_history', 'history_id = ' . intval($history_id)))
		{
	...
	{
		return Application::model()->update('user_action_history', array(
			'add_time' => time()
		), 'history_id = ' . intval($history_id));
	}
	...
	public static function get_action_by_where($where = null, $limit = 20, $show_anonymous = false, $order = 'add_time DESC')
	{
		...
		if ($user_action_history = Application::model()->fetch_all('user_action_history', $where, $order, $limit))
		{
			foreach ($user_action_history AS $key => $val)
			{
				$history_ids[] = $val['history_id'];
	...
	public static function get_actions_fresh_by_where($where = null, $limit = 20, $show_anonymous = false)
	{
		...
		if ($action_history = Application::model()->query_all("SELECT history_id FROM " . get_table('user_action_history_fresh') . " WHERE " . $where . " ORDER BY add_time DESC", $limit))
		{
	...
	{
		$user_link_attr = 'class="icb-user-name" data-id="' . $uid . '"';
		$user_profile_url = 'user/' . $uid;

		if ($associate_topic_info)
		{
			$topic_link_attr = 'class="icb-topic-name" data-id="' . $associate_topic_info['topic_id'] . '"';
		}
		...
		switch ($action)
		{
			case self::ADD_QUESTION:
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> 在 <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题发起了一个问题');
				}
				else if ($associate_question_info['anonymous'])
				{
					$action_string = Application::lang()->_t('匿名用户') . ' ' . Application::lang()->_t('发起了问题');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('发起了问题');
				}
				break;

			case self::ADD_REQUESTION_FOCUS:
				$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('关注了该问题');;
				break;

			case self::ANSWER_QUESTION:
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题新增了一个回答');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('回答了问题');
				}
				break;
			...
			case self::ADD_AGREE: // '增加赞同'
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题添加了一个回复赞同');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('赞同了该回复');
				}
				break;

			case self::ADD_LIKE_PROJECT: // '增加赞同'
				$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('喜欢了该活动');
				break;
			...
			case self::ADD_TOPIC : // '添加话题',
				if ($associate_topic_info)
				{
					...
					{
						$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('将该问题添加到');
						...
						{
							$action_string .= Application::lang()->_t('等') . ' ';
						}

						$action_string .= Application::lang()->_t('话题');
					}
					else
					{
						$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('将该问题添加到') . ' <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题');
					}
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('添加了一个话题');
				}
				break;

			case self::ADD_TOPIC_FOCUS : // '关注话题',
				$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('关注话题');
				break;

			case self::ADD_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> 在 <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题发表了文章');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('发表了文章');
				}
				break;
			...
			case self::ADD_AGREE_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> 在 <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题添加了一个文章赞同');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('赞同了该文章');
				}
				break;
			...
			case self::ADD_COMMENT_ARTICLE :
				if ($associate_topic_info)
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> 在 <a href="' . $topic_url . '" ' . $topic_link_attr . '>' . $associate_topic_info['topic_title'] . '</a> ' . Application::lang()->_t('话题添加了一个文章评论');
				}
				else
				{
					$action_string = '<a href="' . $user_profile_url . '" ' . $user_link_attr . '>' . $user_name . '</a> ' . Application::lang()->_t('评论了该文章');
				}
				break;
		}
	...
	{
		if ($action_history = Application::model()->fetch_all('user_action_history', $where))
		{
			foreach ($action_history AS $key => $val)
			{
				Application::model()->delete('user_action_history_data', 'history_id = ' . $val['history_id']);
				Application::model()->delete('user_action_history_fresh', 'history_id = ' . $val['history_id']);
			}

			$action_history = Application::model()->delete('user_action_history', $where);
		}
	}

	public static function set_fold_action_history($answer_id, $fold = 1)
	{
		Application::model()->update('user_action_history', array(
			'fold_status' => $fold
		...
		if ($fold == 1)
		{
			if ($action_history = Application::model()->fetch_all('user_action_history', 'associate_type IN(' . self::CATEGORY_QUESTION . ',' . self::CATEGORY_ANSWER . ') AND associate_action = ' . self::ANSWER_QUESTION . ' AND associate_attached = ' . intval($answer_id)))
			{
				foreach ($action_history AS $key => $val)
				{
					Application::model()->delete('user_action_history_fresh', 'history_id = ' . $val['history_id']);
