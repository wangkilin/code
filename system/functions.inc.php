...
function date_friendly($timestamp, $time_limit = 604800, $out_format = 'Y-m-d H:i', $formats = null, $time_now = null)
{
    ...
    if ($formats == null)
    {
        $formats = array('YEAR' => Application::lang()->_t('%s 年前'), 'MONTH' => Application::lang()->_t('%s 月前'), 'DAY' => Application::lang()->_t('%s 天前'), 'HOUR' => Application::lang()->_t('%s 小时前'), 'MINUTE' => Application::lang()->_t('%s 分钟前'), 'SECOND' => Application::lang()->_t('%s 秒前'));
    }
...
function & loadClass($class, $params=null)
{
    ...
    if (class_exists($class) === FALSE) {
        $file = INC_PATH . preg_replace('/_+/', '/', $class) . '.php';
...
function _show_error($exception_message)
{
    ...
    if ($exception_message)
    {
        ...
        $errorBlock = "<div " . (Application::config()->get('system')->debug ? "" : "style='display:none'") . " id='exception_message'>"
                    . "<textarea rows='15' onfocus='this.select()'>{$exception_message}</textarea>"
                    . "</div>";
    }
...
{
    return Application::config()->get('database')->prefix . $name;
}
...
function get_setting($varname = null, $permission_check = true)
{
    ...
    if ($settings = Application::$settings) {
        // Application::session()->permission 是指当前用户所在用户组的权限许可项，在 users_group 表中，你可以看到 permission 字段
        if ($permission_check AND $settings['upload_enable'] == 'Y') {
            if (Application::session()) {
                if (! Application::session()->permission['upload_attach']) {
...
function get_login_cookie_hash($user_name, $password, $salt, $uid, $hash_password = true)
{
    ...
    return Application::crypt()->encode(json_encode(array(
        'uid' => $uid,
        'user_name' => $user_name,
        'password' => $password
    )), $auth_hash_key);
...
{
    return Application::form()->valid_post_hash($hash);
}
...
{
    if (! Application::session()->client_info)
    {
        return false;
    }

    return Application::form()->new_post_hash();
}
...
{
    //$str = preg_replace('/\[attach\]([0-9]+)\[\/attach]/', '<i>** ' . Application::lang()->_t('插入的附件') . ' **</i>', $str);
...
{
    if (!class_exists('Application', false)) {
        return loadClass('core_lang')->translate($string, $replace);
    } else {
...
function is_mobile($ignore_cookie = false)
{
    ...
    if (preg_match('/playstation|ipad|ucweb/i', $user_agent) ) {
        return false;
    }
...
/**
 * 统一格式化上传文件变更尺寸后的文件名
 * @param unknown $sizeInfo
 * @return string
 */
function getUploadPicFileNameBySize($sizeInfo)
{
    $upload_data = Application::upload()->data();
    $picFileName = getResizedPicFileName($sizeInfo,
...
function getResizedPicFileName ($sizeInfo, $path, $filename, $ext)
{
    $sizePart = '_' . getSizePartInPicFileName ($sizeInfo);
    $picFileName = $path . str_replace($ext, $sizePart . $ext, $filename);
