...
rm -f $LOCAL_STORE_DIR/MAX_ARTICLE_ID.txt


if [[ "$DB_PASSWORD" = "" ]]; then
    # 1. 数据库备份
    mysqldump -h $DB_HOST -u $DB_USERNAME $DB_DATABASE icb_posts_index --column-statistics=0 --no-create-info  -w "post_id>${ARTICLE_ID}"  > $LOCAL_STORE_DIR/article_post_great_than_${ARTICLE_ID}.sql
    ...
    # 1. 数据库备份
    mysqldump -h $DB_HOST -u $DB_USERNAME -p$DB_PASSWORD $DB_DATABASE icb_posts_index --column-statistics=0 --no-create-info  -w "post_id>${ARTICLE_ID}"  > $LOCAL_STORE_DIR/article_post_great_than_${ARTICLE_ID}.sql
...
# 1.1 打包数据库备份文件
cd $LOCAL_STORE_DIR
tar zcf $LOCAL_STORE_DIR/article_great_than_${ARTICLE_ID}.tgz  article_great_than_${ARTICLE_ID}.sql article_post_great_than_${ARTICLE_ID}.sql
# 1.2 打包后，将刚生成的sql文件删除，只保留打包文件，释放空间。
rm -f $LOCAL_STORE_DIR/article_great_than_${ARTICLE_ID}.sql $LOCAL_STORE_DIR/article_post_great_than_${ARTICLE_ID}.sql
