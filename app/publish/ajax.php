...
class ajax extends BaseController
{
    ...
    public function attach_upload_action()
    {
        ...
        Application::upload()->initialize(array(
            ...
            'upload_path'   => get_setting('upload_dir') . '/' . $item_type . '/' . gmdate('Ymd', APP_START_TIME),
            'is_image'      => FALSE,
        ...
        if (Application::upload()->get_error())
        {
            ...
            {
                default:
                    die("{'error':'错误代码: " . Application::upload()->get_error() . "'}");
                break;
        ...
        {
            $output['class_name'] = $this->model('publish')->getCssClassByFileName(basename($upload_data['full_path']));
        }
    ...
    public function question_attach_edit_list_action()
    {
        ...
        if ($question_attach = $this->model('publish')->getAttachListByItemTypeAndId('question', $_POST['question_id']))
        {
            foreach ($question_attach as $attach_id => $val)
            {
                $question_attach[$attach_id]['class_name'] = $this->model('publish')->getCssClassByFileName($val['file_name']);

                $question_attach[$attach_id]['delete_link'] = get_js_url('/publish/ajax/remove_attach/attach_id-' . Application::crypt()->encode(json_encode(array(
                    'attach_id' => $attach_id,
                    'access_key' => $val['access_key']
                ))));
    ...
    public function answer_attach_edit_list_action()
    {
        ...
        if ($answer_attach = $this->model('publish')->getAttachListByItemTypeAndId('answer', $_POST['answer_id']))
        {
            foreach ($answer_attach as $attach_id => $val)
            {
                $answer_attach[$attach_id]['class_name'] = $this->model('publish')->getCssClassByFileName($val['file_name']);
                $answer_attach[$attach_id]['delete_link'] = get_js_url('/publish/ajax/remove_attach/attach_id-' . Application::crypt()->encode(json_encode(array(
                    'attach_id' => $attach_id,
                    'access_key' => $val['access_key']
                ))));
    ...
    public function modify_question_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题已锁定, 不能编辑')));
        }
        ...
        if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_question'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限编辑这个问题')));
            }
        }

        if (!$_POST['category_id'] AND get_setting('category_enable') == 'Y')
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('请选择分类')));
        }

        if (cjk_strlen($_POST['question_content']) < 5)
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题标题字数不得少于 5 个字')));
        }

        if (get_setting('question_title_limit') > 0 AND cjk_strlen($_POST['question_content']) > get_setting('question_title_limit'))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题标题字数不得大于') . ' ' . get_setting('question_title_limit') . ' ' . Application::lang()->_t('字节')));
        }
        ...
        // 解析出问题所属的it技能： category-1, tag-1
        $tagOrCategoryInfo = explode('-', $_POST['category_id']);
        $question_info['type']    = $tagOrCategoryInfo[0];
        ...
        if ($this->user_id != $question_info['published_uid'])
        {
            ...
            $this->model('notify')->send($this->user_id, $question_info['published_uid'], notifyModel::TYPE_MOD_QUESTION, notifyModel::CATEGORY_QUESTION, $question_info['question_id'], array(
    ...
    public function publish_question_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限发布问题')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('请输入问题标题')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请选择问题分类')));
        }
        ...
        if (get_setting('question_title_limit') > 0 AND cjk_strlen($_POST['question_content']) > get_setting('question_title_limit'))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题标题字数不得大于 %s 字节', get_setting('question_title_limit'))));
        }
        ...
        if ($_POST['topics'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('单个问题话题数量最多为 %s 个, 请调整话题数量', get_setting('question_topics_limit'))));
            }
        }

        if (!$_POST['topics'] AND get_setting('new_question_force_add_topic') == 'Y')
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请为问题添加话题')));
        }
        ...
        if ($_POST['weixin_media_id'])
        {
            ...
            $weixin_pic_url = Application::cache()->get('weixin_pic_url_' . md5($_POST['weixin_media_id']));

            if (!$weixin_pic_url)
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('图片已过期或 media_id 无效')));
            }
            ...
            if (!$file)
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('远程服务器忙')));
            }

            if (is_array($file) AND $file['errmsg'])
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('获取图片失败，错误为: %s', $file['errmsg'])));
            }

            Application::upload()->initialize(array(
                'allowed_types' => get_setting('allowed_upload_types'),
                'upload_path' => get_setting('upload_dir') . '/questions/' . gmdate('Ymd', APP_START_TIME),
                'is_image' => TRUE,
                'max_size' => get_setting('upload_size_limit')
            ));

            Application::upload()->do_upload($_POST['weixin_media_id'] . '.jpg', $file);

            $upload_error = Application::upload()->get_error();
            ...
            if ($upload_error)
            {
                ...
                {
                    default:
                        H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('保存图片失败，错误为: %s', $upload_error)));

                        break;

                    case 'upload_invalid_filetype':
                        H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('保存图片失败，本站不允许上传 jpeg 格式的图片')));

                        break;

                    case 'upload_invalid_filesize':
                        H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('图片尺寸过大, 最大允许尺寸为 %s KB', get_setting('upload_size_limit'))));
            ...
            if (!$upload_data)
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('保存图片失败，请与管理员联系')));
            }
    ...
    public function publish_article_action()
    {
        if (!$this->user_info['permission']['publish_article']) {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限发布文章')));
        }
        ...
        if ($_POST['topics'])
        {
            ...
            if (get_setting('question_topics_limit') AND sizeof($_POST['topics']) > get_setting('question_topics_limit'))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('单个文章话题数量最多为 %s 个, 请调整话题数量', get_setting('question_topics_limit'))));
            }
        }
        if (get_setting('new_question_force_add_topic') == 'Y' AND !$_POST['topics'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请为文章添加话题')));
        }
    ...
    public function modify_article_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('文章已锁定, 不能编辑')));
        }
        ...
        if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_article'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限编辑这个文章')));
            }
        }
        ...
        if (get_setting('question_title_limit') > 0 AND cjk_strlen($_POST['title']) > get_setting('question_title_limit'))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('文章标题字数不得大于') . ' ' . get_setting('question_title_limit') . ' ' . Application::lang()->_t('字节')));
        }
    ...
    public function save_related_link_action()
    {
        ...
        if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'])
        {
            if ($question_info['published_uid'] != $this->user_id)
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限执行该操作')));
            }
        }

        if (substr($_POST['link'], 0, 7) != 'http://' AND substr($_POST['link'], 0, 8) != 'https://')
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('链接格式不正确')));
        }
