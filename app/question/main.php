...
class main extends BaseController
{
	...
	public function index_action()
	{
		...
		if (is_digits($_GET['rf']) and $_GET['rf'])
		{
			...
			{
				$redirect_message[] = Application::lang()->_t('从问题 %s 跳转而来', '<a href="' . get_js_url('/question/' . $_GET['rf'] . '?rf=false') . '">' . $from_question['question_content'] . '</a>');
			}
		}
		...
		else if ($question_info['redirect'])
		{
			if ($target_question)
			{
				$message = Application::lang()->_t('此问题将跳转至') . ' <a href="' . get_js_url('/question/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']) . '">' . $target_question['question_content'] . '</a>';
		...
		$question_info['user_info'] = $this->model('account')->getUserById($question_info['published_uid'], true);
		...
		if ($_GET['column'] != 'log')
		{
			...
			if (get_setting('answer_unique') == 'Y')
			{
				...
				{
					View::assign('user_answered', 1);
				}
				else
				{
					View::assign('user_answered', 0);
				}
			}

			View::assign('answers', $answers);
			View::assign('answer_count', $answer_count);
		}
		...
		{
			$related_topics = $this->model('question')->getUserRelatedTopics($question_info['question_content']);
		...
		View::assign('question_related_list', $this->model('question')->get_related_question_list($question_info['question_id'], $question_info['question_content']));
		...
		if ($_GET['column'] == 'log')
		{
			$this->crumb(Application::lang()->_t('日志'), '/question/id-' . $question_info['question_id'] . '__column-log');
		}
		else
		{
		...
		View::set_meta('keywords', implode(',', $this->model('system')->analysis_keyword($question_info['question_content'])));

		View::set_meta('description', $question_info['question_content'] . ' - ' . cjk_substr(str_replace("\r\n", ' ', strip_tags($question_info['question_detail'])), 0, 128, 'UTF-8', '...'));
		...
		View::output('question/index');
	}

	public function index_square_action()
	{
		$this->crumb(Application::lang()->_t('问题'), '/question/');

		$this->_prepareDataByCheckingTplFile('question/square', 'question');
		...
		View::assign('posts_list', $question_list);
		View::assign('question_list_bit', View::output('index/ajax/list', false));

		View::output('question/square');
	}
	...
	public function unverify_modify_action()
	{
		...
		{
			H::redirect_msg(Application::lang()->_t('问题不存在'), '/');
		}
		...
		H::redirect_msg(Application::lang()->_t('取消确认修改成功, 正在返回...'), '/question/id-' . $_GET['question_id'] . '__column-log__rf-false');
	}
	...
	public function verify_modify_action()
	{
		...
		H::redirect_msg(Application::lang()->_t('确认修改成功, 正在返回...'), '/question/id-' . $_GET['question_id'] . '__column-log__rf-false');
	}
}
