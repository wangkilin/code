...
class ajax extends BaseController
{
    ...
    {
        $this->getAttachListByItemTypeAndId('question', $_POST['question_id']);
    }
    /**
     * 获取问题回答附件
     */
    ...
    {
        $this->getAttachListByItemTypeAndId('answer', $_POST['answer_id']);
    }
    ...
    public function uninterested_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题不存在')));
        }
            ...
            foreach($focus_users_info as $key => $val)
            {
                if ($val['uid'] == $question_info['published_uid'] and $question_info['anonymous'] == 1)
                {
                    $focus_users[$key] = array(
                        'uid' => 0,
                        'user_name' => Application::lang()->_t('匿名用户'),
                ...
                else
                {
                    $focus_users[$key] = array(
                        ...
                        'url' => get_js_url('/user/' . $val['url_token'])
                    );
                }
            }
        }
    ...
    public function save_invite_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('问题不存在或已被删除')));
        }

        if (!$invite_user_info = $this->model('account')->getUserById($_POST['uid']))
        {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不能邀请自己回复问题')));
        }

        if ($this->user_info['integral'] < 0 and get_setting('integral_system_enabled') == 'Y')
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你的剩余积分已经不足以进行此操作')));
        }

        if ($this->model('answer')->has_answer_by_uid($_POST['question_id'], $invite_user_info['uid']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('该用户已经回答过该问题')));
        }

        if ($question_info['published_uid'] == $invite_user_info['uid'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不能邀请问题的发起者回答问题')));
        }

        if ($this->model('question')->has_question_invite($_POST['question_id'], $invite_user_info['uid']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('该用户已接受过邀请')));
        }

        if ($this->model('question')->has_question_invite($_POST['question_id'], $invite_user_info['uid'], $this->user_id))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('已邀请过该用户')));
        }
        ...
        if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($invite_user_info['uid']) AND $invite_user_info['weixin_settings']['QUESTION_INVITE'] != 'N')
        {
        ...
        $notification_id = $this->model('notify')->send($this->user_id, $invite_user_info['uid'], notifyModel::TYPE_INVITE_QUESTION, notifyModel::CATEGORY_QUESTION, intval($_POST['question_id']), array(
    ...
    public function fetch_share_data_action()
    {
        switch ($_GET['type'])
        {
            case 'question':
                ...
                $message = Application::lang()->_t('我看到一个不错的问题, 想和你分享:') . ' ' . $question_info['question_content'] . ' ' . $url;
            break;

            case 'answer':
                $answer_info = $this->model('answer')->get_answer_by_id($_GET['item_id']);

                $user_info = $this->model('account')->getUserById($answer_info['uid']);
                ...
                {
                    $user_info['user_name'] = Application::lang()->_t('匿名用户');
                }

                $message = Application::lang()->_t('我看到一个不错的问题, 想和你分享:') . ' ' . $question_info['question_content'] . ' - ' . $user_info['user_name'] . ": " . $answer_info['answer_content'] . ' ' . $url;
            break;
            ...
            case 'article':
                ...
                $message = Application::lang()->_t('我看到一个不错的文章, 想和你分享:') . ' ' . $article_info['title'] . ": " . $article_info['message'] . ' ' . $url;
            break;
        }
    ...
    public function save_answer_comment_action()
    {
        if (! $_GET['answer_id'])
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('回复不存在')));
        }

        if (!$this->user_info['permission']['publish_comment'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有发表评论的权限')));
        }

        if (trim($_POST['message']) == '')
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('请输入评论内容')));
        }

        if (get_setting('comment_limit') > 0 AND cjk_strlen($_POST['message']) > get_setting('comment_limit'))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('评论内容字数不得超过 %s 字节', get_setting('comment_limit'))));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不能评论锁定的问题')));
        }
    ...
    public function get_answer_comments_action()
    {
        ...
        $user_infos = $this->model('account')->getUsersByIds(fetch_array_value($comments, 'uid'));
        ...
        View::assign('question', $this->model('question')->get_question_info_by_id($answer_info['question_id']));
        View::assign('answer_info', $answer_info);
        View::assign('comments', $comments);

        if (is_mobile())
        {
            View::output("m/ajax/question_comments");
        }
        else
        {
            View::output("question/ajax/comments");
        }
    }
    ...
    public function get_question_comments_action()
    {
        ...
        View::assign('question', $this->model('question')->get_question_info_by_id($_GET['question_id']));
    ...
    public function answer_vote_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('不能对自己发表的回复进行投票')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('投票数据错误, 无法进行投票')));
        }
    ...
    public function question_thanks_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不能感谢自己的问题')));
        }

        if ($this->model('question')->question_thanks($_POST['question_id'], $this->user_id, $this->user_info['user_name']))
        {
            $this->model('notify')->send($this->user_id, $question_info['published_uid'], notifyModel::TYPE_QUESTION_THANK, notifyModel::CATEGORY_QUESTION, $_POST['question_id'], array(
                'question_id' => intval($_POST['question_id']),
                'from_uid' => $this->user_id
            ));
    ...
    public function question_answer_rate_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('不能评价自己发表的回复')));
        }

        if ($_POST['type'] == 'thanks' AND $this->model('answer')->user_rated('thanks', $_POST['answer_id'], $this->user_id))
        {
            H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('已感谢过该回复, 请不要重复感谢')));
        }
        ...
        if ($this->model('answer')->user_rate($_POST['type'], $_POST['answer_id'], $this->user_id, $this->user_info['user_name']))
        {
            if ($answer_info['uid'] != $this->user_id)
            {
                $this->model('notify')->send($this->user_id, $answer_info['uid'], notifyModel::TYPE_ANSWER_THANK, notifyModel::CATEGORY_QUESTION, $answer_info['question_id'], array(
    ...
    public function save_answer_action()
    {
        ...
        if ($question_info['lock'] AND ! ($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_moderator']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('已经锁定的问题不能回复')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不能回复自己发布的问题，你可以修改问题内容')));
        }

        // 判断是否已回复过问题
        if ((get_setting('answer_unique') == 'Y') AND $this->model('answer')->has_answer_by_uid($question_info['question_id'], $this->user_id))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('一个问题只能回复一次，你可以编辑回复过的回复')));
        }
        ...
        else
        {
            ...
            {
                $answer_info['attachs'] = $this->model('publish')->getAttachListByItemTypeAndId('answer', $answer_id, 'min');
            ...
            {
                H::ajax_json_output(Application::RSM(array(
                    'ajax_html' => View::output('m/ajax/question_answer', false)
                ), 1, null));
            }
            else
            {
                H::ajax_json_output(Application::RSM(array(
                    'ajax_html' => View::output('question/ajax/answer', false)
                ), 1, null));
            }
    ...
    public function update_answer_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('答案不存在')));
        }
        ...
        if ($_POST['do_delete'])
        {
            ...
            if ($this->user_id != $answer_info['uid'])
            {
                $this->model('notify')->send($this->user_id, $answer_info['uid'], notifyModel::TYPE_REMOVE_ANSWER, notifyModel::CATEGORY_QUESTION, $answer_info['question_id'], array(
                    'from_uid' => $this->user_id,
                    'question_id' => $answer_info['question_id']
                ));
            }
        ...
        if ($answer_info['uid'] != $this->user_id and ! $this->user_info['permission']['is_administortar'] and ! $this->user_info['permission']['is_moderator'])
        {
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你没有权限编辑这个回复')));
		}
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('已经超过允许编辑的时限')));
		}
	...
	public function log_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('指定问题不存在')));
		}
		...
		View::assign('unverified_modify', $unverified_modify);
		View::assign('question_info', $question_info);

		View::assign('list', $log_list);

		View::output('question/ajax/log');
	}
	...
	public function redirect_action()
	{
		...
		if ($question_info['lock'] AND ! ($this->user_info['permission']['is_administortar'] or $this->user_info['permission']['is_moderator']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('锁定的问题不能设置重定向')));
		}
		...
		if ((!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator']) AND $this->user_info['permission']['function_interval'] AND ((time() - Application::cache()->get('function_interval_timer_redirect_' . $this->user_id)) < $this->user_info['permission']['function_interval']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('灌水预防机制已经打开, 在 %s 秒内不能操作', $this->user_info['permission']['function_interval'])));
		}
		...
		{
			$this->model('notify')->send($this->user_id, $question_info['published_uid'], notifyModel::TYPE_REDIRECT_QUESTION, notifyModel::CATEGORY_QUESTION, $_POST['item_id'], array(
				'from_uid' => $this->user_id,
				'question_id' => intval($_POST['item_id'])
			));
		}

		Application::cache()->set('function_interval_timer_redirect_' . $this->user_id, time(), 86400);
	...
	public function email_invite_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('请填写正确的 Email')));
		}
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('此 E-mail 已接收过邀请')));
		}
		...
		H::ajax_json_output(Application::RSM(null, 1, Application::lang()->_t('邀请成功')));
	}
	...
	public function remove_question_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('对不起, 你没有删除问题的权限')));
		}
	...
	public function remove_comment_action()
	{
		...
		if (! $_GET['comment_id'])
		{
			H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('评论不存在')));
		}
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('你没有权限删除该评论')));
		}
	...
	public function answer_force_fold_action()
	{
		...
		if (! $answer_info['force_fold'])
		{
			...
			if (! $this->model('integral')->fetch_log($answer_info['uid'], 'ANSWER_FOLD_' . $answer_info['answer_id']))
			{
				...
				$this->model('integral')->process($answer_info['uid'], 'ANSWER_FOLD_' . $answer_info['answer_id'], get_setting('integral_system_config_answer_fold'), Application::lang()->_t('回复折叠') . ' #' . $answer_info['answer_id']);
			}

			H::ajax_json_output(Application::RSM(array(
				'action' => 'fold'
			), 1, Application::lang()->_t('强制折叠回复')));
		}
		else
		{
			$this->model('answer')->update_answer_by_id($_POST['answer_id'], array(
	...
	public function get_report_reason_action()
	{
		...
		H::ajax_json_output(Application::RSM($data, 1));
	}
	...
	public function save_report_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('请填写举报理由')));
		}
		...
		$this->model('message')->send_message($this->user_id, $recipient_uid, Application::lang()->_t('有新的举报, 请登录后台查看处理: %s', get_js_url('/admin/question/report_list/')));

		H::ajax_json_output(Application::RSM(null, 1, Application::lang()->_t('举报成功')));
	}
	...
	public function set_best_answer_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('回答不存在')));
		}
		...
		{
			H::ajax_json_output(Application::RSM(null, - 1, Application::lang()->_t('问题已经存在最佳回复')));
		}
