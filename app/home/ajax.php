...
class ajax extends Controller
{
        ...
        if ($_GET['filter'] == 'focus')
        {
            if ($result = $this->model('question')->get_user_focus($this->user_id, (intval($_GET['page']) * $this->per_page) . ", {$this->per_page}"))
            {
                ...
                foreach ($result as $key => $val)
                {
                    ...
                    {
                        $user_info_list[$val['published_uid']] = $this->model('account')->getUserById($val['published_uid'], true);
                    }
    ...
    public function draft_action()
    {
        ...
        if (is_mobile())
        {
            View::output('m/ajax/draft');
        }
        else
        {
            View::output('home/ajax/draft');
        }
    }
    ...
    public function invite_action()
    {
        ...
        if (is_mobile())
        {
            View::output('m/ajax/invite');
        }
        else
        {
            View::output('home/ajax/invite');
        }
    }
}