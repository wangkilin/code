...
class ajax extends BaseController
{
    ...
    public function tag_list_action ()
    {
    	...
    	$tagList = $this->getTagByConditions($conditions, null, $perPage);
	...
	public function edit_topic_action()
	{
		if (!($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_moderator']))
		{
			...
			{
				H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('锁定的话题不能编辑')));
			}
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请填写话题描述')));
		}
	...
	public function save_related_topic_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入话题标题')));
		}

		if (strstr($_POST['topic_title'], '/') OR strstr($_POST['topic_title'], '-') OR strstr($_POST['topic_title'], '&'))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题标题不能包含 / - &')));
		}

		if (get_setting('topic_title_limit') > 0 AND cjk_strlen($topic_title) > get_setting('topic_title_limit'))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题标题字数不得超过 %s 字节', get_setting('topic_title_limit'))));
		}

		if (! $related_id = $this->model('topic')->saveTopic($topic_title, $this->user_id, $this->user_info['permission']['create_topic']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题已锁定或没有创建话题权限')));
		}

		if (!$this->model('topic')->save_related_topic($_GET['topic_id'], $related_id))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('已经存在相同推荐话题')));
		}
	...
	public function upload_topic_pic_action()
	{
		...
		{
			foreach(Application::config()->get('image')->topic_thumbnail AS $key => $val)
			{
				...
				@unlink(get_setting('upload_dir') . '/topic/' . str_replace(Application::config()->get('image')->topic_thumbnail['min']['w'] . '_' . Application::config()->get('image')->topic_thumbnail['min']['h'], $val['w'] . '_' . $val['h'], $topic_info['topic_pic']));
			}
		...
		$this->model('topic')->update_topic($this->user_id, $_GET['topic_id'], null, null, gmdate('Ymd', APP_START_TIME) . '/' . basename($thumb_file['min']));

		echo htmlspecialchars(json_encode(array(
			'success' => true,
			'thumb' => get_setting('upload_url') . '/topic/' . gmdate('Ymd', APP_START_TIME) . '/' . basename($thumb_file['mid'])
		)), ENT_NOQUOTES);
	}
	...
	public function set_parent_id_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不允许为根话题设置根话题')));
		}

		if ($topic_info['topic_id'] == intval($_POST['parent_id']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('不允许将根话题设置为自己')));
		}
	...
	public function save_url_token_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题别名只允许输入英文或数字')));
		}

		if ($this->model('topic')->check_url_token($_POST['url_token'], $topic_info['topic_id']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题别名已经被占用请更换一个')));
		}

		if (preg_match("/^[\d]+$/i", $_POST['url_token']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题别名不允许为纯数字')));
		}
	...
	public function merge_topic_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题合并不能与自己合并')));
		}
		...
		if ($topic_info['merged_id'])
		{
			...
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('该话题已经与 %s 合并', $merged_topic_info['topic_title'])));
		}
	...
	public function remove_topic_relation_action()
	{
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('指定的项目不存在')));
		}
		...
		switch ($_POST['type'])
		{
			...
			case 'article':
				...
				{
					H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('指定项目不存在')));
				}
	...
	public function set_article_topic_relation_action()
	{
		...
		switch ($_POST['type'])
		{
			case 'question':
				...
				{
					H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('锁定问题不能添加话题')));
				}
			break;
			...
			case 'article':
				...
				{
					H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('锁定文章不能添加话题')));
				}
			break;
		}
		...
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('单个问题或文章话题数量最多为 %s 个, 请调整话题数量', get_setting('question_topics_limit'))));
		}

		if (! $topic_id = $this->model('topic')->saveTopic($_POST['topic_title'], $this->user_id, $this->user_info['permission']['create_topic']))
		{
			H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('话题已锁定或没有创建话题权限, 不能添加话题')));
		}
