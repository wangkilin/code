...
class openid_twitter extends Controller
{
    ...
    public function setup()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站未开通 Twitter 登录'), '/');
        }
    }

    public function bind_action()
    {
        if (Application::session()->twitter_request_token)
        {
            $twitter_request_token = Application::session()->twitter_request_token;

            unset(Application::session()->twitter_request_token);
        }

        if (Application::session()->twitter_user)
        {
            $twitter_user_info = Application::session()->twitter_user;
        ...
        if ($_GET['denied'])
        {
            H::redirect_msg(Application::lang()->_t('授权失败'), '/account/login/');
        }
        ...
        if ($this->user_id)
        {
            ...
            if ($twitter_user)
            {
                H::redirect_msg(Application::lang()->_t('此账号已绑定 Twitter 账号'), '/account/login/');
            }
        }
        ...
        if ($_GET['oauth_token'])
        {
            if (!$twitter_user_info)
            {
                ...
                {
                    H::redirect_msg(Application::lang()->_t('oauth token 不一致'), '/account/login/');
                }

                if (!$_GET['oauth_verifier'])
                {
                    H::redirect_msg(Application::lang()->_t('oauth verifier 为空'), '/account/login/');
                }
            ...
            if (!$twitter_user_info)
            {
                H::redirect_msg(Application::lang()->_t('Twitter 登录失败，用户信息不存在'), '/account/login/');
            }
            ...
            if ($this->user_id)
            {
                if ($twitter_user)
                {
                    H::redirect_msg(Application::lang()->_t('此 Twitter 账号已被绑定'), '/account/login/');
                }
            ...
            else
            {
                if ($twitter_user)
                {
                    $user = $this->model('account')->getUserById($twitter_user['uid']);

                    if (!$user)
                    {
                        $this->model('openid_twitter')->unbind_account($twitter_user['uid']);

                        H::redirect_msg(Application::lang()->_t('本地用户不存在'), '/account/login/');
                    }
                    ...
                    else
                    {
                        ...
                        {
                            Application::session()->valid_email = $user['email'];
                        }
                    }
                ...
                else
                {
                    ...
                    {
                        case 'close':
                            H::redirect_msg(Application::lang()->_t('本站目前关闭注册'), '/account/login/');

                            break;

                        case 'invite':
                            H::redirect_msg(Application::lang()->_t('本站只能通过邀请注册'), '/account/login/');

                            break;

                        case 'weixin':
                            H::redirect_msg(Application::lang()->_t('本站只能通过微信注册'), '/account/login/');

                            break;
                    }

                    Application::session()->twitter_user = $twitter_user_info;

                    $this->crumb(Application::lang()->_t('完善资料'), '/account/login/');

                    View::assign('register_url', 'account/ajax/twitter/register/');

                    View::assign('user_name', Application::session()->twitter_user['name']);

                    View::import_css('css/register.css');

                    View::output('account/openid/callback');
                }
            }
        }
        else
        {
            ...
            Application::session()->twitter_request_token = $this->model('openid_twitter')->request_token;
