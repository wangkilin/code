...
class openid_facebook extends Controller
{
    ...
    public function setup()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站未开通 Facebook 登录'), '/');
        }
    }
    ...
    public function bind_action()
    {
        ...
        {
            $facebook_user_info = Application::session()->facebook_user;
        ...
        if ($this->user_id)
        {
            ...
            if ($facebook_user)
            {
                H::redirect_msg(Application::lang()->_t('此账号已绑定 Facebook 账号'), '/account/login/');
            }
        }
        ...
        if ($_GET['code'])
        {
            ...
            {
                H::redirect_msg(Application::lang()->_t('Facebook 登录失败，用户信息不存在'), '/account/login/');
            }
            ...
            if ($this->user_id)
            {
                if ($facebook_user)
                {
                    H::redirect_msg(Application::lang()->_t('此 Facebook 账号已被绑定'), '/account/login/');
                }
            ...
            else
            {
                if ($facebook_user)
                {
                    $user = $this->model('account')->getUserById($facebook_user['uid']);
                ...
                else
                {
                    ...
                    Application::session()->facebook_user = $facebook_user_info;

                    $this->crumb(Application::lang()->_t('完善资料'), '/account/login/');

                    View::assign('register_url', 'account/ajax/facebook/register/');

                    View::assign('user_name', Application::session()->facebook_user['name']);

                    View::assign('email', Application::session()->facebook_user['email']);
        ...
        else
        {
            ...
            {
                H::redirect_msg(Application::lang()->_t('本接口只有当伪静态启用时才可以使用，请联系管理员开启伪静态'), '/account/login/');
            }
