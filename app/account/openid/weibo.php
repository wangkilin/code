...
class openid_weibo extends Controller
{
    ...
    public function setup()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站未开通微博登录'), '/');
        }
    }
    ...
    public function bind_action()
    {
        ...
        {
            $weibo_user_info = Application::session()->weibo_user;
        ...
        if ($this->user_id)
        {
            ...
            if ($weibo_user)
            {
                H::redirect_msg(Application::lang()->_t('此账号已绑定微博账号'), '/account/login/');
            }
        }
        ...
        if ($_GET['code'])
        {
            ...
            if (!$weibo_user_info)
            {
                H::redirect_msg(Application::lang()->_t('微博登录失败，用户信息不存在'), '/account/login/');
            }
            ...
            if ($this->user_id)
            {
                if ($weibo_user)
                {
                    H::redirect_msg(Application::lang()->_t('此微博账号已被绑定'), '/account/login/');
                }
            ...
            else
            {
                if ($weibo_user)
                {
                    $user = $this->model('account')->getUserById($weibo_user['uid']);
                ...
                else
                {
                    ...
                    Application::session()->weibo_user = $weibo_user_info;

                    $this->crumb(Application::lang()->_t('完善资料'), '/account/login/');

                    View::assign('register_url', 'account/ajax/weibo/register/');

                    View::assign('user_name', Application::session()->weibo_user['screen_name']);
