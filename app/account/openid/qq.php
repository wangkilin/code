...
class openid_qq extends Controller
{
    ...
    public function setup()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站未开通 QQ 登录'), '/');
        }
    }
    ...
    public function bind_action()
    {
        ...
        {
            $qq_user_info = Application::session()->qq_user;
        ...
        if ($this->user_id)
        {
            ...
            if ($qq_user)
            {
                H::redirect_msg(Application::lang()->_t('此账号已绑定 QQ 账号'), '/account/login/');
            }
        }
        ...
        if ($_GET['code'])
        {
            ...
            if (!$qq_user_info)
            {
                H::redirect_msg(Application::lang()->_t('QQ 登录失败，用户信息不存在'), '/account/login/');
            }
            ...
            if ($this->user_id)
            {
                if ($qq_user)
                {
                    H::redirect_msg(Application::lang()->_t('此 QQ 账号已被绑定'), '/account/login/');
                }
            ...
            else
            {
                if ($qq_user)
                {
                    $user = $this->model('account')->getUserById($qq_user['uid']);
                ...
                else
                {
                    ...
                    Application::session()->qq_user = $qq_user_info;

                    $this->crumb(Application::lang()->_t('完善资料'), '/account/login/');

                    View::assign('register_url', 'account/ajax/qq/register/');

                    View::assign('user_name', Application::session()->qq_user['nickname']);
