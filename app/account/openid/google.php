...
class openid_google extends Controller
{
    ...
    public function setup()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站未开通 Google 登录'), '/');
        }
    }
    ...
    public function bind_action()
    {
        ...
        {
            $google_user_info = Application::session()->google_user;
        ...
        if ($this->user_id)
        {
            ...
            if ($google_user)
            {
                H::redirect_msg(Application::lang()->_t('此账号已绑定 Google 账号'), '/account/login/');
            }
        }
        ...
        if ($_GET['code'])
        {
            ...
            if (!$google_user_info)
            {
                H::redirect_msg(Application::lang()->_t('Google 登录失败，用户信息不存在'), '/account/login/');
            }
            ...
            if ($this->user_id)
            {
                if ($google_user)
                {
                    H::redirect_msg(Application::lang()->_t('此 Google 账号已被绑定'), '/account/login/');
                }
            ...
            else
            {
                if ($google_user)
                {
                    $user = $this->model('account')->getUserById($google_user['uid']);
                ...
                else
                {
                    ...
                    Application::session()->google_user = $google_user_info;

                    $this->crumb(Application::lang()->_t('完善资料'), '/account/login/');

                    View::assign('register_url', 'account/ajax/google/register/');

                    View::assign('user_name', Application::session()->google_user['name']);

                    View::assign('email', Application::session()->google_user['email']);
