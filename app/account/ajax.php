...
class ajax extends Controller
{
    ...
    public function check_username_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('用户名不符合规则')));
        }

        if ($this->model('account')->check_username($_POST['username']))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('用户名已被注册')));
        }
    ...
    public function check_email_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('邮箱地址已被使用')));
        }
    ...
    public function register_process_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('本站目前关闭注册')));
        }
        else if (get_setting('register_type') == 'invite' AND !$_POST['icode'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('本站只能通过邀请注册')));
        }
        else if (get_setting('register_type') == 'weixin')
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('本站只能通过微信注册')));
        }
        ...
        if ($_POST['icode'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('邀请码无效或与邀请邮箱不一致')));
            }
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('用户名包含无效字符')));
        }
        else if ($this->model('account')->check_username_sensitive_words($_POST['user_name']) OR trim($_POST['user_name']) != $_POST['user_name'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('用户名中包含敏感词或系统保留字')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('你必需同意用户协议才能继续')));
        }

        // 检查验证码
        if (!Application::captcha()->is_validate($_POST['seccode_verify']) AND get_setting('register_seccode') == 'Y')
        {
        ...
        if (get_setting('ucenter_enabled') == 'Y')
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, $result));
            }
        }
        else
        {
        ...
        if (isset($_POST['sex']))
        {
            ...
            // 更新主表
            $this->model('account')->setAccountInfos($update_data, $uid);
        ...
        {
            $follow_users = $this->model('account')->getUserById(HTTP::get_cookie('fromuid'));
        }
        ...
        {
            Application::session()->valid_email = $user_info['email'];
    ...
    public function login_process_action()
    {
        ...
        else
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, get_setting('close_notice')));
            }
    ...
    {
        H::ajax_json_output(Application::RSM(null, 1, nl2br(get_setting('register_agreement'))));
    }
    ...
    public function welcome_message_template_action()
    {
        ...
        View::output('account/ajax/welcome_message_template');
    }

    public function welcome_get_topics_action()
    {
        if ($topics_list = $this->model('topic')->getTopicList(null, 'RAND()', 8))
        {
            foreach ($topics_list as $key => $topic)
            {
                $topics_list[$key]['isFollowed'] = $this->model('topic')->isTopicFollowed($this->user_id, $topic['topic_id']);
            }
        }
        View::assign('topics_list', $topics_list);

        View::output('account/ajax/welcome_get_topics');
    }
    ...
    public function welcome_get_users_action()
    {
        ...
        View::assign('users_list', $users_list);

        View::output('account/ajax/welcome_get_users');
    }
    ...
    public function save_draft_action()
    {
        ...
        H::ajax_json_output(Application::RSM(null, 1, Application::lang()->_t('已保存草稿, %s', date('H:i:s', time()))));
    }
    ...
    public function modify_unvalid_email_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('不允许已认证邮箱用户更改邮箱')));
        }
        ...
        Application::session()->valid_email = strtolower($_POST['email']);

        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('邮箱更改成功, 请前往邮箱接收验证邮件')));
    }
    ...
    public function send_valid_mail_action()
    {
        if (!$this->user_id)
        {
            if ( H::valid_email(Application::session()->valid_email))
            {
                $this->user_info = $this->model('account')->get_user_info_by_email(Application::session()->valid_email);
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('错误, 用户没有提供 E-mail')));
        }

        if ($this->user_info['valid_email'] == 1)
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('用户邮箱已经认证')));
        }

        $this->model('active')->new_valid_email($this->user_id);

        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('邮件发送成功')));
    }
    ...
    public function valid_email_active_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('激活失败, 无效的链接')));
        }
        ...
        if (!$user_info = $this->model('account')->getUserById($active_data['uid']))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('激活失败, 无效的链接')));
        }
        ...
        if ($this->model('active')->active_code_active($_POST['active_code'], 'VALID_EMAIL'))
        {
            ...
            {
                unset(Application::session()->valid_email);
            }
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('激活成功, 请等待管理员审核账户')));
            }
        }
    }
    ...
    public function request_find_password_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1,  Application::lang()->_t('请填写正确的邮箱地址')));
        }
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1,  Application::lang()->_t('邮箱地址错误或帐号不存在')));
        }

        $this->model('active')->new_find_password($user_info['uid']);

        Application::session()->find_password = $user_info['email'];
    ...
    public function find_password_modify_action()
    {
        ...
        if ($active_data)
        {
            if ($active_data['active_time'] OR $active_data['active_ip'])
            {
                H::ajax_json_output(Application::RSM(null, -1,  Application::lang()->_t('链接已失效，请重新找回密码')));
            }
        }
        else
        {
        ...
        if (!$_POST['password'])
        {
            H::ajax_json_output(Application::RSM(null, -1,  Application::lang()->_t('请输入密码')));
        }

        if ($_POST['password'] != $_POST['re_password'])
        {
            H::ajax_json_output(Application::RSM(null, -1,  Application::lang()->_t('两次输入的密码不一致')));
        }
        ...
        unset(Application::session()->find_password);

        H::ajax_json_output(Application::RSM(array(
            'url' => get_js_url('/account/login/'),
        ), 1, Application::lang()->_t('密码修改成功, 请返回登录')));
    }
    ...
    public function avatar_upload_action()
    {
        ...
        if (Application::upload()->get_error()) {
            switch (Application::upload()->get_error()) {
                ...
                case 'upload_invalid_filesize':
                    H::ajax_json_output(Application::RSM(null, '-1', _t('文件尺寸过大, 最大允许尺寸为%sKB', get_setting('upload_avatar_size_limit'))));
                    break;
        ...
        // 更新主表
        $this->model('account')->setAccountInfos($update_data, $this->user_id);
    ...
    function add_edu_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入学校名称')));
        }

        if (!$_POST['departments'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入院系')));
        }

        if ($_POST['education_years'] == Application::lang()->_t('请选择') OR !$_POST['education_years'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请选择入学年份')));
        }

        if (preg_match('/\//is', $_POST['school_name']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('学校名称不能包含 /')));
        }

        if (preg_match('/\//is', $_POST['departments']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('院系名称不能包含 /')));
        }

        if (get_setting('auto_create_social_topics') == 'Y')
        {
            $this->model('topic')->saveTopic($_POST['school_name']);
            $this->model('topic')->saveTopic($_POST['departments']);
        }
        ...
        {
            $this->model('integral')->process($this->user_id, 'UPDATE_EDU', round((get_setting('integral_system_config_profile') * 0.2)), Application::lang()->_t('完善教育经历'));
        }
    ...
    function add_work_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入公司名称')));
        }

        if (!$_POST['job_id'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请选择职位')));
        }

        if (!$_POST['start_year'] OR !$_POST['end_year'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请选择工作时间')));
        }

        if (preg_match('/\//is', $_POST['company_name']))
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('公司名称不能包含 /')));
        }

        if (get_setting('auto_create_social_topics') == 'Y')
        {
            $this->model('topic')->saveTopic($_POST['company_name']);
        }
        ...
        {
            $this->model('integral')->process($this->user_id, 'UPDATE_WORK', round((get_setting('integral_system_config_profile') * 0.2)), Application::lang()->_t('完善工作经历'));
        }
    ...
    public function privacy_setting_action()
    {
        ...
        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('隐私设置保存成功')));
    }
    ...
    public function profile_setting_action()
    {
        if (!$this->user_info['user_name'] OR $this->user_info['user_name'] == $this->user_info['email'] AND $_POST['user_name'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', $check_result));
            }
        }
        ...
        if ($_POST['url_token'] AND $_POST['url_token'] != $this->user_info['url_token'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('你距离上次修改个性网址未满 30 天')));
            }

            if (!preg_match("/^(?!__)[a-zA-Z0-9_]+$/i", $_POST['url_token']))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('个性网址只允许输入英文或数字')));
            }

            if ($this->model('account')->check_url_token($_POST['url_token'], $this->user_id))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('个性网址已经被占用请更换一个')));
            }

            if (preg_match("/^[\d]+$/i", $_POST['url_token']))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('个性网址不允许为纯数字')));
            }
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('已经存在相同的姓名, 请重新填写')));
        }
        ...
        if (! H::valid_email($this->user_info['email']))
        {
            if (! H::valid_email($_POST['email']))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入正确的 E-Mail 地址')));
            }

            if ($this->model('account')->check_email($_POST['email']))
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('邮箱已经存在, 请使用新的邮箱')));
            }
        ...
        if ($_POST['common_email'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('请输入正确的常用邮箱地址')));
            }
        ...
        {
            $this->model('integral')->process($this->user_id, 'UPDATE_SIGNATURE', round((get_setting('integral_system_config_profile') * 0.1)), Application::lang()->_t('完善一句话介绍'));
        }
        ...
        {
            $this->model('integral')->process($this->user_id, 'UPDATE_CONTACT', round((get_setting('integral_system_config_profile') * 0.1)), Application::lang()->_t('完善联系资料'));
        }
        ...
        if (get_setting('auto_create_social_topics') == 'Y')
        {
            if ($_POST['city'])
            {
                $this->model('topic')->saveTopic($_POST['city']);
            }

            if ($_POST['province'])
            {
                $this->model('topic')->saveTopic($_POST['province']);
            }
        }
        ...
        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('个人资料保存成功')));
    }
    ...
    public function integral_log_action()
    {
        if ($log = $this->model('integral')->fetch_all('integral_log', 'uid = ' . $this->user_id, 'time DESC', (intval($_GET['page']) * 10) . ', 10'))
        {
            ...
            View::assign('log', $log);
            View::assign('log_detail', $this->model('integral')->parse_log_item($parse_items));
        }

        View::output('account/ajax/integral_log');
    }
    ...
    public function verify_action()
    {
        if ($this->is_post() AND !$this->user_info['verified'])
        {
            if (trim($_POST['name']) == '')
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入真实姓名或企业名称')));
            }

            if (trim($_POST['reason']) == '')
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入申请认证说明')));
            }
            ...
            //$this->model('message')->send_message($this->user_id, $recipient_uid, Application::lang()->_t('有新的认证请求, 请登录后台查看处理: %s', get_js_url('/admin/user/verify_approval_list/')));
        }
    ...
    {
        Application::cache()->delete('user_recommend_' . $this->user_id);
    }
    ...
    public function unbinding_weixin_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('当前帐号没有绑定 Email, 不允许解除绑定')));
        }

        if (get_setting('register_type') == 'weixin')
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('当前系统设置不允许解除绑定')));
        }
    ...
    public function weixin_login_process_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('当前微信公众号暂不支持此功能')));
        }
    ...
    public function complete_profile_action()
    {
        if ($this->user_info['email'])
        {
            H::ajax_json_output(Application::RSM(null, '-1', Application::lang()->_t('当前帐号已经完善资料')));
        }
