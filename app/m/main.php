...
class main extends BaseController
{
    ...
    public function home_action()
    {
        ...
        $this->crumb(Application::lang()->_t('动态'), '/m/');
    ...
    {
        $this->crumb(Application::lang()->_t('我关注的问题'), '/m/focus/');
    ...
    {
        $this->crumb(Application::lang()->_t('邀请我回答的问题'), '/m/invite/');

        View::output('m/invite');
    }
    ...
    public function inbox_action()
    {
        if ($_GET['dialog_id'])
        {
            ...
            {
                H::redirect_msg(Application::lang()->_t('指定的站内信不存在'), '/m/inbox/');
            }
            ...
            $this->crumb(Application::lang()->_t('私信对话') . ': ' . $recipient_user['user_name'], '/m/inbox/dialog_id-' . intval($_GET['dialog_id']));

            View::assign('body_class', 'active');

            View::assign('list', $list_data);

            View::assign('recipient_user', $recipient_user);

            View::output('m/inbox_read');
        }
        else
        {
            $this->crumb(Application::lang()->_t('私信'), '/m/inbox/');
    ...
    public function inbox_new_action()
    {
        ...
        $this->crumb(Application::lang()->_t('新私信'), '/m/inbox_new/');
    ...
    public function question_action()
    {
        ...
        if (is_digits($_GET['rf']) and $_GET['rf'])
        {
            ...
            {
                $redirect_message[] = Application::lang()->_t('从问题') . ' <a href="' . get_js_url('/m/question/' . $_GET['rf'] . '?rf=false') . '">' . $from_question['question_content'] . '</a> ' . Application::lang()->_t('跳转而来');
            }
        }
        ...
        if ($question_info['redirect'] and ! $_GET['rf'])
        {
            ...
            {
                $redirect_message[] = Application::lang()->_t('重定向目标问题已被删除, 将不再重定向问题');
            }
        }
        else if ($question_info['redirect'])
        {
            if ($target_question)
            {
                $message = Application::lang()->_t('此问题将跳转至') . ' <a href="' . get_js_url('/m/question/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']) . '">' . $target_question['question_content'] . '</a>';
                ...
                {
                    $message .= '&nbsp; (<a href="javascript:;" onclick="AWS.ajax_request(G_BASE_URL + \'/question/ajax/redirect/\', \'item_id=' . $question_info['question_id'] . '\');">' . Application::lang()->_t('撤消重定向') . '</a>)';
                }
        ...
        {
            $question_info['attachs'] = $this->model('publish')->getAttachListByItemTypeAndId('question', $question_info['question_id'], 'min');
        ...
        if (get_setting('answer_unique') == 'Y')
        {
            ...
            {
                View::assign('user_answered', TRUE);
            }
        }

        $question_info['question_detail'] = FORMAT::parse_attachs(nl2br(FORMAT::parse_bbcode($question_info['question_detail'])));

        View::assign('question_id', $question_info['question_id']);
        View::assign('question_info', $question_info);
        View::assign('question_focus', $this->model('question')->has_focus_question($question_info['question_id'], $this->user_id));
        View::assign('question_topics', $this->model('topic')->get_topics_by_item_id($question_info['question_id'], 'question'));
        ...
        if ($this->user_id)
        {
            View::assign('question_thanks', $this->model('question')->get_question_thanks($question_info['question_id'], $this->user_id));

            View::assign('invite_users', $this->model('question')->get_invite_users($question_info['question_id']));

            //View::assign('user_follow_check', $this->model("follow")->user_follow_check($this->user_id, $question_info['published_uid']));

            if ($this->user_info['draft_count'] > 0)
            {
                View::assign('draft_content', $this->model('draft')->get_data($question_info['question_id'], 'answer', $this->user_id));
            }
        }
        ...
        if ($answer_ids)
        {
            $answer_agree_users = $this->model('answer')->getVoteUsersByAnswerIds($answer_ids);

            $answer_vote_status = $this->model('answer')->checkUserVoteAnswerStatus($answer_ids, $this->user_id);
            ...
            $answer_attachs = $this->model('publish')->getAttachesByItemTypeAndIds('answer', $has_attach_answer_ids, 'min');
        }
        ...
        View::assign('answers_list', $answers);
        ...
        View::assign('question_related_list', $question_related_list);

        View::assign('question_related_links', $this->model('related')->get_related_links('question', $question_info['question_id']));
        ...
        if ($this->user_id)
        {
            ...
            if ($helpful_users = $this->model('topic')->get_helpful_users_by_topic_ids($question_topic_ids, 12))
            {
                ...
                View::assign('helpful_users', $helpful_users);
            }
        }
        ...
        if ($_GET['page'] < $total_page)
        {
            ...
            View::assign('next_page', $_GET['page']);
        }
    ...
    public function login_action()
    {
        ...
        View::assign('body_class', 'explore-body');
        View::assign('return_url', strip_tags($return_url));

        $this->crumb(Application::lang()->_t('登录'), '/m/login/');
    ...
    public function register_action()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('本站只能通过邀请注册'));
        }
        ...
        $this->crumb(Application::lang()->_t('注册'), '/m/register/');

        View::assign('body_class', 'explore-body');

        View::output('m/register');
    }

    public function find_password_action()
    {
        $this->crumb(Application::lang()->_t('找回密码'), '/m/find_password/');

        View::output('m/find_password');
    }
    ...
    public function find_password_success_action()
    {
        ...
        $this->crumb(Application::lang()->_t('找回密码'), '/m/find_password_success/');

        View::output('m/find_password_success');
    }
    ...
    public function find_password_modify_action()
    {
        ...
        View::output('m/find_password_modify');
    }
    ...
    public function index_action()
    {
        ...
        $this->crumb(Application::lang()->_t('发现'), '/m/');
        ...
        if ($_GET['feature_id'])
        {
            View::assign('feature_info', $this->model('feature')->get_feature_by_id($_GET['feature_id']));
        }
    ...
    public function people_action()
    {
        ...
        else
        {
            ...
            {
                HTTP::redirect('/m/user/' . $user['url_token']);
            }
        ...
        View::assign('fans_list', $this->model('follow')->get_user_fans($user['uid'], 20));
        View::assign('friends_list', $this->model('follow')->get_user_friends($user['uid'], 20));
        View::assign('focus_topics', $this->model('topic')->get_focus_topic_list($user['uid'], 8));

        View::assign('user_actions_questions', $this->model('actions')->get_user_actions($user['uid'], 15, ACTION_LOG::ADD_QUESTION, $this->user_id));
        View::assign('user_actions_answers', $this->model('actions')->get_user_actions($user['uid'], 15, ACTION_LOG::ANSWER_QUESTION, $this->user_id));

        $this->crumb(Application::lang()->_t('%s 的个人主页', $user['user_name']), '/m/user/' . $user['url_token']);
    ...
    public function people_square_action()
    {
        ...
        $this->crumb(Application::lang()->_t('用户列表'), '/m/user/');
        ...
        else
        {
            ...
            View::assign('pagination', Application::pagination()->initialize(array(
                'base_url' => get_js_url('/m/user/group_id-' . $_GET['group_id']),
    ...
    public function search_action()
    {
        ...
        View::output('m/search');
    }

    public function topic_square_action()
    {
        $this->crumb(Application::lang()->_t('话题广场'), '/m/topic/');

        View::assign('topics_hot_list', $this->model('topic')->getTopicList(null, 'discuss_count DESC', 5));

        View::output('m/topic_square');
    }
    ...
    public function topic_action()
    {
        ...
        $this->crumb(Application::lang()->_t('话题'), '/m/topic/');
    ...
    {
        $this->crumb(Application::lang()->_t('通知'), '/m/notifications/');

        View::output('m/notifications');
    }

    public function draft_action()
    {
        $this->crumb(Application::lang()->_t('草稿'), '/m/draft/');
    ...
    {
        $this->crumb(Application::lang()->_t('设置'), '/m/settings/');
    ...
    public function publish_action()
    {
        if ($_GET['id'])
        {
            ...
            if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_question'])
            {
                ...
                {
                    H::redirect_msg(Application::lang()->_t('你没有权限编辑这个问题'), '/m/question/' . $_GET['id']);
                }
            }
        ...
        {
            $weixin_pic_url = Application::cache()->get('weixin_pic_url_' . md5(base64_decode($_GET['weixin_media_id'])));

            if (!$weixin_pic_url)
            {
                H::redirect_msg(Application::lang()->_t('图片已过期或 media_id 无效'));
            }

            View::assign('weixin_media_id', $_GET['weixin_media_id']);

            View::assign('weixin_pic_url', $weixin_pic_url);
        }
        else
        {
    ...
    public function article_action()
    {
        ...
        $article_info['vote_users'] = $this->model('article')->getVoteUsersByArticleId('article', $article_info['id'], null, 10);
        ...
        View::output('m/article');
    }

    public function article_square_action()
    {
        $this->crumb(Application::lang()->_t('文章'), '/m/article/');
        ...
        if ($_GET['category'])
        {
            View::assign('category_info', $this->model('system')->get_category_info($_GET['category']));
        }

        View::output('m/article_square');
    }

    public function nearby_people_action()
    {
        $this->crumb(Application::lang()->_t('附近的人'), '/m/nearby_user/');

        if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($this->user_id))
        {
            if (!$near_by_users = $this->model('people')->get_near_by_users($weixin_user['longitude'], $weixin_user['latitude'], $this->user_id, 20))
            {
                H::redirect_msg(Application::lang()->_t('你的附近暂时没有人'));
            }
        }
        else
        {
            H::redirect_msg(Application::lang()->_t('请先绑定微信或打开地理位置分享'));
        }

        View::assign('near_by_users', $near_by_users);

        View::output('m/nearby_people');
    }

    public function nearby_question_action()
    {
        $this->crumb(Application::lang()->_t('附近的问题'), '/m/nearby_question/');
        ...
        if ($weixin_user = $this->model('openid_weixin_weixin')->getUserById($this->user_id))
        {
            ...
            {
                H::redirect_msg(Application::lang()->_t('你的附近暂时没有问题'));
            }
        }
        else
        {
        ...
        View::assign('near_by_questions', $near_by_questions);

        View::output('m/nearby_question');
    }

    public function verify_action()
    {
        $this->crumb(Application::lang()->_t('申请认证'), '/m/verify/');

        View::assign('body_class', 'active');

        View::assign('verify_apply', $this->model('verify')->fetch_apply($this->user_id));

        View::output('m/verify');
    }

    public function favorite_action()
    {
        $this->crumb(Application::lang()->_t('我的收藏'), '/m/favorite/');

        View::output('m/favorite');
    }
}
