...
class weixin extends Controller
{
    ...
    public function redirect_action()
    {
        ...
        if ($_GET['code'] AND get_setting('weixin_app_id') AND get_setting('weixin_app_secret'))
        {
            ...
            if ($access_token = $this->model('openid_weixin_weixin')->get_sns_access_token_by_authorization_code($_GET['code']))
            {
                ...
                {
                    $user_info = $this->model('account')->getUserById($weixin_user['uid']);
    ...
    public function authorization_action()
    {
        ...
        {
            H::redirect_msg(Application::lang()->_t('此功能只适用于通过微信认证的服务号'));
        }
            ...
            if ($access_token)
            {
                ...
                if ($access_user['errcode'])
                {
                    if ($access_user['errcode'] == 48001)
                    {
                        ...
                        H::redirect_msg(Application::lang()->_t('当前微信没有绑定社区帐号, 请返回进行绑定后访问本内容'));
                    }
                    else
                    {
                ...
                if (!$access_user['nickname'])
                {
                    ...
                    {
                        H::redirect_msg(Application::lang()->_t('您当前没有关注本公众号主账号, 无法使用身份认证功能'));
                    }

                    H::redirect_msg(Application::lang()->_t('您当前没有关注本公众号, 无法使用全部功能'));
                }
                ...
                if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_openid($access_token['openid']))
                {
                    ...
                    {
                        H::redirect_msg(Application::lang()->_t('绑定微信成功'), '/m/');
                    }
                }
                ...
                if (get_setting('register_type') == 'weixin')
                {
                    ...
                    {
                        H::redirect_msg(Application::lang()->_t('注册失败,请返回重新操作'));
                    }
                }
                else
                {
                    ...
                    View::assign('access_user', $access_user);


                    if (get_setting('register_type') != 'close' AND get_setting('register_type') != 'invite')
                    {
                        View::assign('register_url', $this->model('openid_weixin_weixin')->get_oauth_url(get_js_url('/m/weixin/register/redirect-' . urlencode($_GET['redirect'])), 'snsapi_userinfo'));
                    }

                    View::assign('body_class', 'explore-body');

                    View::output('m/weixin/authorization');
                }
            }
            else
            {
    ...
    public function register_action()
    {
        ...
        if ($_GET['code'] AND get_setting('weixin_app_id'))
        {
            ...
            if (!$access_user['nickname'])
            {
                H::redirect_msg(Application::lang()->_t('获取用户信息失败'));
            }
