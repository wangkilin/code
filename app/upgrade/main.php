...
class main extends Controller
{
	...
	public function setup()
	{
		...
		if (!is_really_writable(INC_PATH) AND !defined('IN_SAE'))
		{
			H::redirect_msg(Application::lang()->_t('目录 %s 无法写入, 请修改目录权限', INC_PATH));
		}
		...
		{
			H::redirect_msg(Application::lang()->_t('当前升级器只支持 2.0 与以上版本升级, 你当前版本太低, 请下载 2.0 - 2.5.1 版先进行升级'));
		}
		...
		else if (!in_array($this->db_version, $this->versions) AND $_GET['act'] != 'final' AND $_GET['act'] != 'script')
		{
			...
			{
				H::redirect_msg(Application::lang()->_t('您的程序已经是最新版本'));
			}
			else
			{
				H::redirect_msg(Application::lang()->_t('无法定位您的程序版本, 请手动执行升级, Build: %s', $this->db_version));
			}
		}

		View::assign('static_url', G_STATIC_URL);
	...
	public function index_action()
	{
		...
		View::assign('db_version', $this->db_version);
		View::output('install/upgrade');
	}
	...
	public function sql_action()
	{
		...
		if (trim($sql_query))
		{
			...
			echo str_replace(array('[#DB_PREFIX#]', '[#DB_ENGINE#]'), array(Application::config()->get('database')->prefix, $this->db_engine), $sql_query);
			die;
		}
	}
	...
	public function run_action()
	{
		foreach ($this->versions AS $version)
		{
			...
			if (trim($sql_query))
			{
				...
				{
					View::assign('sql_error', $sql_error);
					View::assign('version', $version);
					View::output('install/upgrade_fail');
					die;
				}
			}
	...
	public function script_action()
	{
		...
		H::redirect_msg(Application::lang()->_t('正在执行升级脚本 %s, 请耐心等待...', $script_version), '/upgrade/script/' . rand(100000, 666666));
	}

	public function final_action()
	{
		H::redirect_msg(Application::lang()->_t('升级完成, 您的程序已经是最新版本, 如遇搜索功能异常, 请进入后台更新搜索索引') . '<!-- Analytics --><img src="http://www.wecenter.com/analytics/?build=' . G_VERSION_BUILD . '&amp;site_name=' . urlencode(get_setting('site_name')) . '&amp;base_url=' . urlencode(base_url()) . '&amp;php=' . PHP_VERSION . '" alt="" width="1" height="1" /><!-- / Analytics -->', '/');
	}
}
