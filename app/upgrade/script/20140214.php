...
switch ($_GET['post_type'])
{
	...
	case 'question':
		if ($questions_list = $this->model('question')->fetch_page('question', null, 'question_id ASC', $_GET['page'], 300))
		{
			...
			H::redirect_msg(Application::lang()->_t('正在升级问题数据库') . ', ' . Application::lang()->_t('批次: %s', $_GET['page']), '/upgrade/script/post_type-question__page-' . ($_GET['page'] + 1));
			die;
		}
		else
		{
	...
	case 'article':
		if ($articles_list = $this->model('question')->fetch_page('article', null, 'id ASC', $_GET['page'], 300))
		{
			...
			H::redirect_msg(Application::lang()->_t('正在升级文章数据库') . ', ' . Application::lang()->_t('批次: %s', $_GET['page']), '/upgrade/script/post_type-article__page-' . ($_GET['page'] + 1));
			die;
		}
		else
		{
