...
class ajax extends Controller
{
	...
	public function user_actions_action()
	{
		...
		if ($_GET['actions'] == '201')
		{
			View::output($template_dir . '/ajax/user_actions_questions_201');
		}
		else if ($_GET['actions'] == '101')
		{
			View::output($template_dir . '/ajax/user_actions_questions_101');
		}
		else
		{
			View::output($template_dir . '/ajax/user_actions');
		}
	}
	...
	public function user_info_action()
	{
		...
		else if (!$user_info = $this->model('account')->getUserById($_GET['uid'], ture))
		{
		...
		H::ajax_json_output(array(
			...
			'url' => get_js_url('/user/' . $user_info['url_token']),
	...
	public function topics_action()
	{
		if ($topic_list = $this->model('topic')->get_focus_topic_list($_GET['uid'], (intval($_GET['page']) * $this->per_page) . ", {$this->per_page}") AND $this->user_id)
		{
			...
			if ($topic_ids)
			{
				$topic_focus = $this->model('topic')->getFollowedFromTopicIds($this->user_id, $topic_ids);

				foreach ($topic_list as $key => $val)
				{
					$topic_list[$key]['isFollowed'] = $topic_focus[$val['topic_id']];
				}
			}
		}
