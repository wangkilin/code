...
class ajax extends Controller
{
	...
	public function search_result_action()
	{
		...
		if ($this->user_id AND $search_result)
		{
			foreach ($search_result AS $key => $val)
			{
				switch ($val['type'])
				{
					...
					case 'topics':
						$search_result[$key]['focus'] = $this->model('topic')->isTopicFollowed($this->user_id, $val['search_id']);
		...
		if (is_mobile())
		{
			View::output('m/ajax/search_result');
		}
		else
		{
			View::output('search/ajax/search_result');
		}
	}
