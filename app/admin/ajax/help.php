...
class help extends AdminController
{
    ...
    public function save_chapter_action()
    {
        if (!$_POST['title'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请填写章节标题')));
        }
        ...
        if ($_POST['id'])
        {
            ...
            if (!$chapter_info)
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('指定章节不存在')));
            }
        }
        ...
        else
        {
            ...
            if (!$id)
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('保存失败')));
            }
        }
        ...
        if ($_FILES['icon']['name'])
        {
            ...
            foreach (Application::config()->get('image')->chapter_thumbnail as $key => $val)
            {
    ...
    public function save_chapter_sort_action()
    {
        ...
        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('章节排序已自动保存')));
    }
    ...
    public function remove_chapter_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('删除章节失败')));
        }
    ...
    public function save_data_sort_action()
    {
        ...
        H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('数据排序已自动保存')));
    }
