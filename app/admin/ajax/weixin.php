...
class weixin extends AdminController
{
    ...
    public function save_third_party_access_rule_status_action()
    {
        if (!$_POST['rule_ids'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请选择要操作的规则')));
        }
    ...
    public function remove_third_party_access_rule_action()
    {
        if (!$_POST['id'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请选择要删除的规则')));
        }

        if(!$this->model('openid_weixin_third')->get_third_party_api_by_id($_POST['id']))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('接入规则不存在')));
        }
    ...
    public function save_third_party_access_rule_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入正确的 URL')));
        }
        ...
        if (!$_POST['token'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入第三方公众平台接口 Token')));
        }
    ...
    public function create_qr_code_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入描述')));
        }
    ...
    public function remove_qr_code_action()
    {
        if (!$_POST['scene_id'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请选择要删除的二维码')));
        }
   ...
   public function save_reply_rule_status_action()
   {
    ...
    public function save_account_action()
    {
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('微信公众平台接口 Token 不能为空')));
        }
        ...
        switch ($_POST['type'])
        {
            case 'add':
                ...
                H::ajax_json_output(Application::RSM(array('url' => get_js_url('/admin/weixin/account/id-' . $account_id)), 1, null));
    ...
    public function send_msg_action()
    {
        ...
        if (!isset($group_name))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('选择的分组不存在')));
        }

        if (!$_POST['main_msg_title'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入封面的标题')));
        }

        if (!$_POST['main_msg_author'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入封面的作者')));
        }

        if (!$_POST['main_msg_content'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入封面的内容')));
        }

        if (!$_POST['main_msg_url'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入封面的原文链接')));
        }

        if ($_POST['show_cover_pic'] != 0 AND $_POST['show_cover_pic'] != 1)
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请选择是否显示封面')));
        }
...
/*
        ...
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请添加要群发的文章或问题')));
        }
*/

        if (count($article_ids) + count($question_ids) > 9)
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('最多可添加 9 个文章和问题')));
        }
        ...
        if ($_FILES['main_msg_img']['error'] === UPLOAD_ERR_OK)
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('只允许上传 bmp/png/jpeg/jpg/gif 格式的图片')));
            }

            if ($_FILES['main_msg_img']['size'] > '2097152')
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('图片最大为 2M')));
            }
        ...
        {
            $main_msg_img = Application::config()->get('weixin')->default_list_image_path;
        }
        ...
        if (isset($error_msg))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('上传封面图失败, 错误信息: %s', $error_msg)));
        }
        ...
        if ($article_ids)
        {
            ...
            if (isset($error_msg))
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('上传文章作者的头像失败, 错误信息: %s', $error_msg)));
            }
        }
        ...
        if ($question_ids)
        {
            ...
            if (isset($error_msg))
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('上传问题作者的头像失败, 错误信息: %s', $error_msg)));
            }
        }
        ...
        if (isset($error_msg))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('上传图文消息失败, 错误为: %s', $error_msg)));
        }
        ...
        if (isset($error_msg))
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('群发任务提交失败, 错误信息: %s', $error_msg)));
        }
        ...
        H::ajax_json_output(Application::RSM(array('url' => get_js_url('/admin/weixin/sent_msg_details/id-' . $msg_id)), 1, null));
    }
}
