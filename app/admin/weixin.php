...
class weixin extends AdminController
{
    ...
    {
        $this->crumb(Application::lang()->_t('自定义回复'), 'admin/weixin/reply/');
        ...
        if (!$account_info)
        {
            H::redirect_msg(Application::lang()->_t('公众账号不存在'));
        }

        View::assign('account_info', $account_info);

        View::assign('rule_list', $this->model('weixin')->fetch_reply_rule_list($account_info['id']));

        View::assign('accounts_list', $accounts_list);

        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/reply'));

        View::output('admin/weixin/reply');
    }

    public function reply_edit_action()
    {
        $this->crumb(Application::lang()->_t('编辑回复规则'), "admin/weixin/reply_edit/");
        ...
        if ($_GET['id'])
        {
            ...
            if (!$rule_info)
            {
                H::redirect_msg(Application::lang()->_t('自定义回复规则不存在'), '/admin/weixin/reply/');
            }

            View::assign('account_id', $rule_info['account_id']);

            View::assign('rule_info', $rule_info);
        }
        else
        {
            ...
            if (!$account_info)
            {
                H::redirect_msg(Application::lang()->_t('公众账号不存在'), '/admin/weixin/reply/');
            }

            View::assign('account_id', $account_info['id']);
        }

        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/reply'));

        View::output('admin/weixin/reply_edit');
    }

    public function mp_menu_action()
    {
        $this->crumb(Application::lang()->_t('菜单管理'), 'admin/weixin/mp_menu/');
        ...
        {
            H::redirect_msg(Application::lang()->_t('此功能不适用于未通过微信认证的订阅号'), '/admin/');
        }
        ...
        View::assign('mp_menu', $accounts_list[$account_id]['weixin_mp_menu']);

        View::assign('accounts_list', $accounts_list);

        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/mp_menu'));

        View::assign('feature_list', $this->model('feature')->get_enabled_feature_list('id DESC', null, null));

        if (get_setting('category_enable') == 'Y')
        {
            View::assign('category_data', json_decode($this->model('system')->build_category_json('question'), true));
        }

        View::assign('reply_rule_list', $this->model('weixin')->fetch_unique_reply_rule_list($account_id));

        View::import_js('js/fileupload.js');

        View::import_js('js/md5.js');

        View::output('admin/weixin/mp_menu');
    }
    ...
    public function save_mp_menu_action()
    {
        ...
        if ($_POST['button'])
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('远程服务器忙,请稍后再试')));
            }
        }
    ...
    public function rule_save_action()
    {
        ...
        if (!$_POST['keyword'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入关键词')));
        }

        if (!$_POST['title'])
        {
            H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('请输入回应内容')));
        }
        ...
        if ($_POST['id'])
        {
            ...
            if (!$rule_info)
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('自定义回复规则不存在')));
            }
        }
        else
        {
            ...
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('公众账号不存在')));
            }

            if ($this->model('weixin')->get_reply_rule_by_keyword($_POST['account_id'], $_POST['keyword']) AND !$_FILES['image']['name'])
            {
                H::ajax_json_output(Application::RSM(null, -1, Application::lang()->_t('已经存在相同的文字回应关键词')));
            }
        }
    ...
    {
        $this->crumb(Application::lang()->_t('公众账号管理'), "admin/weixin/accounts/");
        ...
        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/accounts'));

        View::assign('accounts_list', $accounts_list);

        View::assign('accounts_total', $accounts_total);

        View::output('admin/weixin/accounts');
    }

    public function account_action()
    {
        $this->crumb(Application::lang()->_t('编辑公众账号'), "admin/weixin/accounts/");
        ...
        if ($_GET['id'])
        {
            ...
            if (!$account_info)
            {
                H::redirect_msg(Application::lang()->_t('该账号不存在'), '/admin/weixin/accounts/');
            }
        ...
        View::output('admin/weixin/account');
    }

    public function sent_msgs_list_action()
    {
        $this->crumb(Application::lang()->_t('群发消息列表'), "admin/weixin/sent_msgs_list/");
        ...
        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/sent_msgs_list'));

        View::assign('msgs_list', $msgs_list);

        View::assign('msgs_total', $msgs_total);

        View::output('admin/weixin/sent_msgs_list');
    }

    public function sent_msg_details_action()
    {
        $this->crumb(Application::lang()->_t('查看群发消息'), "admin/weixin/sent_msg_details/");

        $msg_details = $this->model('weixin')->get_msg_details_by_id($_GET['id']);

        if (!$msg_details)
        {
            H::redirect_msg(Application::lang()->_t('群发消息不存在'), '/admin/weixin/sent_msgs_list/');
        }

        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/sent_msgs_list'));

        View::assign('msg_details', $msg_details);

        View::output('admin/weixin/sent_msg_details');
    }

    public function send_msg_batch_action()
    {
        $this->crumb(Application::lang()->_t('群发消息'), "admin/weixin/send_msg_batch/");
        ...
        {
            H::redirect_msg(Application::lang()->_t('此功能只适用于通过微信认证的公众号'), '/admin/weixin/sent_msgs_list/');
        }
        ...
        if (!is_array($groups))
        {
            H::redirect_msg(Application::lang()->_t('获取微信分组失败, 错误为: %s', $groups), '/admin/weixin/sent_msgs_list/');
        }
        ...
        View::output('admin/weixin/send_msg_batch');
    }

    public function qr_code_action()
    {
        $this->crumb(Application::lang()->_t('二维码管理'), "admin/weixin/qr_code/");

        if (get_setting('weixin_account_role') != 'service' OR !get_setting('weixin_app_id') OR !get_setting('weixin_app_secret'))
        {
            H::redirect_msg(Application::lang()->_t('此功能只适用于通过微信认证的服务号'), '/admin/');
        }
        ...
        View::assign('qr_code_list', $qr_code_list);
        ...
        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/qr_code'));

        View::output('admin/weixin/qr_code');
    }

    public function third_party_access_action()
    {
        $this->crumb(Application::lang()->_t('第三方接入'), 'admin/weixin/third_party_access/');
        ...
        if (!isset($account_id))
        {
            H::redirect_msg(Application::lang()->_t('公众账号不存在'), '/admin/weixin/mp_menu/');
        }
        ...
        View::assign('menu_list', $this->model('admin')->fetch_menu_list('weixin/third_party_access'));

        View::output('admin/weixin/third_party_access');
    }

    public function edit_third_party_access_rule_action()
    {
        $this->crumb(Application::lang()->_t('接入规则编辑'), 'admin/weixin/third_party_access/');
        ...
        View::output('admin/weixin/edit_third_party_access_rule');
    }
}
